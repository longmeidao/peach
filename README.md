...
- macOS 菜单栏和 Windows 托盘都有「同步 Ledger」：它先只读判定这次会不会真的复制，
  会复制才停止当前托盘拥有的 HTTP/HTTPS，完成一次安全推送或拉取后再恢复服务；
  共享盘没挂、两侧已经一致或本机不是写入端时直接回话，不动服务。
  发现端口由别的进程占用时拒绝接管。
...
「同步开发进度」和它是两回事，所以是两个按钮：前者走 GitHub 把本地检出快进到
`origin/master` 再让新代码跑起来，后者走共享盘搬账本。两条链路互不兜底——共享盘没挂
不该挡住拉代码，拉代码失败也不该被说成账本出了事。它只做 `merge --ff-only`：工作区有
未提交修改或两边分叉时原样报出来，不会 stash 也不会改写历史。拉到的改动动了菜单栏项
自己的代码时，光重启子服务追不上，它会用 `launchctl kickstart -k` 把菜单栏项一起重启。
...
观看页可按作者、来源和 Rule34.xxx 提供的标签筛选；管理页把同一作者的跨站来源放在一组。
Rule34.xxx 标签不区分大小写，重复添加会复用同一来源，旧重复项由迁移合并且保留条目状态。
作者头像优先从已验证的官方页面取得（当前支持 FANBOX/Pixiv），失败才回退归档站头像。
