...
### macOS 的「本地网络」权限会静默掐掉 mDNS

`peach serve` 从终端起时 `peach.local` 一切正常，改由 LaunchAgent 起之后手机就再也解析不到
——同一个二进制、同一台机器、同样的参数。原因是 macOS 的「本地网络」隐私门：终端起的进程
继承终端已获授权的身份，launchd 起的作业是另一个主体，而且**没有弹窗可点**，多播被静默丢弃。
zeroconf 自认为注册成功，`/healthz` 也照报 `peach.local`，实际一个包都发不出去。

所以发布器现在会自己发一次 mDNS 查询验证，验不过就把状态报成 `unreachable` 并在日志里写清
原因——「自称成功」比直接失败难查得多。

修法二选一：系统设置 → 隐私与安全性 → 本地网络里允许该进程；或者 `--no-mdns` 关掉它，
改用局域网 IP 访问（证书的 SAN 里已经带了本机地址）。
