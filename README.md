...
| 产地区分 | 日本、韩国、国产、欧美各成一类，筛选里可单选或多选，还能列出尚未判定的那批；番号推得出的自动归类，推不出的选中一批一次判完，之后不被自动结果改写 |
...
界面偏好保存在当前浏览器，在右上角「设置」里改：默认排序、JAV 封面用官方封面还是预览图、打开详情是否自动播放、按钮开关和操作回执要不要配一声轻响的「界面音效」，以及把图像模糊降饱和、关掉悬停预览的「SFW 模式」。

同一处的「视频缩略图采集」跟着运行 Peach 的这台电脑走，不是每个浏览器各存一份：选精准（每 10 秒一帧）或粗略（每 30 秒一帧）后它在后台按密度抽帧，拖动进度条时按时间点显示画面。默认关闭，只采集本机磁盘上的片子；网盘上的每抽一帧都要回源拉一次。产物随片子一起在数据管理页清理。

界面采用 Board 风格。设置里的「增加对比度」关闭导航玻璃的透明与折射；「侧栏光晕」是左栏玻璃面上那三枚慢慢漂动的光，可以关掉，其余玻璃面（搜索框、顶栏图标钮、筛选浮层、设置卡的分区导航、媒体库与配色弹层）跟着取前两枚的颜色；左栏底部那枚配色钮上挑光晕配色与强调色，「玻璃原色」那一档让每块玻璃回到自带的反光、颜色跟着明暗主题走，强调色管按钮、焦点环、链接和数据色；设置里的「侧栏光晕」调强度、颗粒感、漂移速度、柔化、大小和三枚光晕各自的颜色，浅色与深色主题都会显示。数量和时间支持范围内自定义，可关闭的功能使用开关控制。
...
测试包可只在本机访问，也可在首次设置中允许同一局域网的设备访问；局域网模式通过
`http://<设置的名字>.local:<端口>` 打开，首次连接时只允许 Windows 防火墙的专用网络。
不需要 Python、Git、Node 或 OpenSSL。数据保存在 `%LOCALAPPDATA%\Peach\peach-data`，与程序目录分开。
