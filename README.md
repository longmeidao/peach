...
| 能力 | 使用方式 |
| --- | --- |
| 浏览与搜索 | 按作品、女优、厂牌、创作者、系列和标签查找视频与图集 |
...
复核名称前提供勾选框，支持 Shift 连选，按候选数量、来源或字段分组。筛选项随复核类别更新，可选择具体分类并全选当前分类；批量操作只处理当前可见的所选项目。选中后，底部浮窗提供数量、共同来源、通过、拒绝和取消选择，与馆藏、关注、标签及回收站共用浮窗布局；点击通过后才会采用。失败项保留选择和错误提示，可重试。身份候选提供来源资料、本地作品样本和文件位置入口，可打开全部作品核对；通过后登记身份判断。

复核分类与筛选栏滚动到顶部后吸附在主导航下方，分组标题跟随吸附，全选本组紧邻标题。吸附栏仅在吸附时显示铺满内容区的背景；卡片标题与当前信息在资料滚动时保持可见。多选浮窗的已选数量使用统一文字样式。

首次设置可选择进入浏览器历史记录导入指南；完成导入后指南隐藏，点击「跳过」会在当前浏览器记住选择，折叠仍保留指南。需要补充记录时使用「口味」页的读取或导入按钮。设置中的「SFW 模式」会模糊、降低饱和度并压暗图片和视频，停止悬停预览，文字保持可见。数据管理中的「检查文件与馆藏记录」覆盖已配置的本地磁盘和网盘，离线来源跳过。首启与配置页提供 CloudDrive 的硬盘、内存、读取块及缓存建议。
...
右上角设置可控制打开本地作品和关注视频详情时是否自动播放，以及离开详情时正在播放的视频是否缩到角落的小窗继续播放。小窗开着时点击其他视频直接在小窗里换片，可拖到四个角、点击标题展开回详情，画面上的播放键两侧还有按设置秒数跳的快退和快进；播放器右键菜单提供循环播放、迷你播放器、画中画、复制视频网址（含当前时间）和播放统计。
