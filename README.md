...
| 能力 | 说明 |
| --- | --- |
| 浏览与搜索 | 按作品、女优、厂牌、创作者、系列和标签查找视频与图集；没有署名人的作品归为「未归属」一类 |
...
界面采用 Board 风格。设置中可选择「使用旧版 UI」并应用刷新；「增加对比度」关闭导航玻璃的透明与折射。数量和时间支持范围内自定义，可关闭的功能使用开关控制。

侧栏顶部选择媒体库，底部切换明暗主题并打开设置。配置中给文件夹填写媒体库名称，同名文件夹归入一库；库选择限定作品列表与筛选项。
...
「管理 → 配置 → 检查更新」比对 GitHub 上的最新测试版，独立包可以在页面里下载并安装，失败时恢复旧版；
也可以退出托盘后手动完整解压新版，保留数据目录。配置页提供卸载，可选是否一并删除 Peach 数据，原始媒体保留。
