...
- **折痕按「切出来的正封形状对不对」认，不按它落在全宽的百分之几**。正封宽高比是
  印刷面的物理常数（DVD 135×190mm ＝ 0.711），本机 637 张实测中位 0.706、1% 分位
  0.686、99% 分位 0.758；折痕在全宽里的位置则随背面留白与书脊厚度飘。所以只在
  `PANEL_ASPECT_MIN`～`PANEL_ASPECT_MAX`（0.68～0.76）这个窗口里取最强的峭壁，窗外
  再强的边也不看：偏左是书脊自己的另一条边，会把整条书脊带进正封；偏右是正封内部
  的强边，会把大标题削掉一截——KBI-036 正是后者：背面一条分栏线与正封里的一道边
  恰好关于中线对称，按「对称即折痕」那套判据就会采信它。
...
- 框就是正封本身：`x0` 是折痕列，右下角就是源图右下角。框里不另取子区域——卡片的
  容器比例只有页面知道，在这一侧先按某个形状切一刀，只会把正封两侧各削掉一圈。
- 默认只算边车缺失、算法版本落后或源图尺寸对不上的封面，所以重探换上更大的图之后
  再跑一遍就会重算；`--redo` 才无条件全算。改了判据要进 `ALGORITHM_VERSION`，
  全库才会重算。
...
- **容器比例定在 0.75，别按单张封面调**：一行卡片必须等高，比例只能是全局一个数。
  切出来的正封宽高比落在 0.670～0.759 之间，0.75 之下只有 5 张要从左边切、最多 1.2%
  （一张 800 宽的封面上是 4 像素），其余两侧留白由 `--cover-blur` 那层模糊背景垫，
  每边中位 3.1%、最大 5.3%。取 0.72 会让 45 张被切掉最多 5.1%，取 0.76 一张不切但
  留白到每边中位 3.7%。
