...
- 页面只用框里的 `x0`：`web/app.js` 的 `posterPanel` 把它换算成 `--panel-clip` 与
  ...
  框里的 2:3 形状是算法的中间产物，卡片不按它取景。
- **容器比例定在 0.75，别按单张封面调**：一行卡片必须等高，比例只能是全局一个数。
  本机 1014 张实测 683 张有正封，正封自己的宽高比中位数 0.706、99% 分位 0.747、
  最大 0.802。0.75 覆盖到 99% 分位，只有 5 张的正封比卡片宽、要从左边切掉最多
  6.5%；其余装得下的两侧留白，由 `--cover-blur` 那层模糊背景垫。取 0.70 会让 624 张
  被切，取 0.80 则留白宽到每边 5.9%。
- 没有折痕数据的封套退回「贴右缘」，取景与算框之前一致，不会变形也不会留白。
