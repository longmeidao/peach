...
- **峰是斜坡最陡的那一列，不是斜坡尽头**。折痕是一道有宽度的斜坡，书脊最后一两列还压在
  峰的右边，按峰切会在正封左缘留下一条竖线。选定后再往右走到梯度落回窗内中位数为止
  （`FOLD_SETTLE_LIMIT`，最多走源图宽的 1%），基线每张各算。实测位移中位 2 列、90% 分位 3 列。
...
- **容器比例定在 0.75，别按单张封面调**：一行卡片必须等高，比例只能是全局一个数。
  切出来的正封宽高比落在 0.667～0.749 之间，全在 0.75 以下，一张都不用从左边切；两侧留白
  由 `--cover-blur` 模糊背景垫，每边中位 3.1%、最大 5.6%（取 0.72 会有 10 张被切掉最多 3.8%）。
