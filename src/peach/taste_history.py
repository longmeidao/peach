...
from .web_logic import LENGTH_TAGS, TECH_TAGS
...
def _taste_analysis(
    ...
    if browser_names:
        points.append({
            ...
            "note": f"来自 {int(history['visits']):,} 次去重访问",
        })
    if peach_names:
        points.append({
            "label": "Peach 印证",
            "text": "、".join(peach_names),
            "note": f"来自 {int(peach['assets']):,} 个有行为项目",
        })
    if shared:
        points.append({
            "label": "共同信号",
            "text": "、".join(shared),
            "note": "浏览与 Peach 两类证据同时出现",
        })
    elif browser_only:
        points.append({
            "label": "待验证",
            "text": browser_only,
            "note": "浏览信号较强，但 Peach 尚无观看印证",
        })
    ...
    if visits >= 1000 and assets >= 50 and explicit >= 10:
        confidence = ("high", "可信度较高", "浏览、观看与明确反馈均已有稳定样本。")
    elif visits >= 1000 and assets >= 20:
        confidence = (
            "medium", "可信度中等",
            f"浏览和观看样本充足，但明确反馈只有 {explicit} 次；不合口味不会用于 Tag 降权。",
        )
    else:
        confidence = ("early", "仍在学习", "样本较少，当前结论只适合用作探索入口。")
    ...
    for category, signal in browser_categories.most_common():
        ...
        explore.append({
            ...
            "detail": (
                f"浏览相关信号 {int(signal):,} 次；当前首页范围可找到 {inventory[tag]:,} 部"
                + ("，已有观看印证。" if corroborated else "，尚未形成观看印证。")
            ),
    ...
    if creator_leads:
        next_steps.append({
            ...
            "note": "这些身份在浏览记录反复出现，但 Peach 尚无观看证据。",
            "route": "/follow-manage",
        })
    if int(peach["coverage"]["untagged"]) or int(peach["coverage"]["unidentified"]):
        next_steps.append({
            ...
            "note": "补齐后，创作者与内容建议会更准确。", "route": "/review",
        })
    if explicit < 10:
        next_steps.append({
            "kind": "feedback", "title": "增加明确反馈",
            "detail": f"当前喜欢 {int(peach['liked'])} · 不合口味 {int(peach['disliked'])}",
            "note": "喜欢用于增强正向证据；不合口味仍只保留在具体项目。", "route": "/",
        })
    return {
        "headline": headline,
        "points": points,
        "confidence": {"level": confidence[0], "label": confidence[1], "note": confidence[2]},
        "explore": explore,
