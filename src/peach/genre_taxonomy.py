...
CONTENT_GENRES: dict[str, str] = {
    ...
    # FC2 卖家把口交的姿势与花样各写成一格：站着的、骑在身上的、不用手的、事后清理的，
    # 说的都是口交。单写的 `仁王立ち`、`お掃除` 在来源里是 `…フェラ` 的省写。
    "お掃除フェラ": "口交", "お掃除": "口交", "フェラ抜き": "口交", "口淫": "口交",
    "ノーハンド": "口交", "ノーハンドフェラ": "口交", "仁王立ち": "口交", "仁王立ちフェラ": "口交",
    "馬乗りフェラ": "口交", "バキュームフェラ": "口交", "激しいフェラ": "口交", "交互フェラ": "口交",
    "Wフェラ": "口交", "ご奉仕フェラ": "口交", "包茎フェラ": "口交", "ゴムフェラ": "口交",
    "フェラ顔": "口交", "フェラポーズ": "口交", "フェラ撮影会": "口交",
    "チンポ舐め": "口交", "玉舐め": "口交", "金玉舐め": "口交", "裏筋": "口交", "裏筋舐め": "口交",
    ...
    "ぶっかけ": "颜射", "Bukkake/Facial": "颜射", "大量顔射": "颜射",
    "Oral Cum": "口爆", "Cum in Mouth": "口爆", "口内発射": "口爆", "口内射精": "口爆",
    "舌上発射": "口爆", "舌上射精": "口爆",
    "Cum Swallowing": "吞精", "Cum Swallow": "吞精", "Swallowing": "吞精",
    "Swallow": "吞精", "ごっくん": "吞精", "飲精": "吞精", "精飲": "吞精",
    # `射精` 说的是射了几回、射了多少，射在哪里另有口爆、颜射、中出内射三格。
    "2回射精": "射精", "大量射精": "射精", "大量発射": "射精", "大量精子": "射精",
    "ザーメン発射": "射精", "大噴射": "射精", "精子を大噴射": "射精", "特濃ザーメン": "射精",
    # `搾り取る` 是女方主动把男方榨到射；FC2 的标签是从标题截的，同一个词留下好几种长短。
    "精子を搾り取": "榨精", "精子を搾り取って": "榨精", "精子を搾り": "榨精", "搾り取": "榨精",
    "ザーメン搾り取": "榨精", "精液搾り": "榨精",
    # `指マン`／`手マン` 是同一个动作的两种写法，来源里两种都出现。
    "Handjob": "手交", "Hand Job": "手交", "Fingering": "手交", "手コキ": "手交",
    "指マン": "手交", "手マン": "手交",
    "Titty Fuck": "乳交", "Tit Fuck": "乳交", "Titjob": "乳交", "Paizuri": "乳交",
    "パイズリ": "乳交", "挟射": "乳交", "馬乗りパイズリ": "乳交", "着衣パイズリ": "乳交",
    "Footjob": "足交", "Foot Job": "足交", "足コキ": "足交",
    "Squirting": "潮吹", "Squirts": "潮吹", "Squirt": "潮吹", "潮吹き": "潮吹", "潮吹き娘": "潮吹",
    "母乳": "母乳", "Lactation": "母乳", "スカトロ": "排泄", "Scat": "排泄", "浣腸": "排泄",
    "フィストファック": "拳交", "フィスト": "拳交", "Fisting": "拳交",
    "W姦": "3P多人", "生姦": "中出内射", "高速騎乗位": "骑乘",
    # `生` 说的是不戴套，和 `生ハメ` 同去向；FC2 标题里写成 `【生中】`。
    "濃厚中出し": "中出内射", "膣内射精": "中出内射", "半外半中出し": "中出内射",
    "ザーメン注入": "中出内射", "生セックス": "中出内射", "生SEX": "中出内射",
    "生挿入": "中出内射", "生": "中出内射",
    "オナホール": "性玩具", "ギャプ萌え": "反差",
    "Masturbation": "自慰", "Masterbation": "自慰", "オナニー": "自慰",
    "Anal Sex": "肛交", "Anal Play": "肛交", "Anal": "肛交",
    "アナル": "肛交", "アナルセックス": "肛交",
    # `アナル中出し` 一个原文只投一个标签，取更能分开馆藏的肛交；`AF`、`ATM` 是圈内缩写。
    "アナルファック": "肛交", "AF": "肛交", "ATM": "肛交", "アナル中出し": "肛交",
    "アナル責め": "肛交", "アナルシリーズ": "肛交",
    "アナル舐め": "毒龙", "亀頭責め": "龟头责", "2穴挿入": "双洞齐插",
    "腋コキ": "腋", "わき": "腋", "腋見せフェチ": "腋",
    "正常位": "传教士", "ピストン": "打桩", "ガン突き": "打桩",
    "Cowgirl": "骑乘", "Reverse Cowgirl": "骑乘", "Girl on Top": "骑乘",
    "Woman on Top": "骑乘", "騎乗位": "骑乘", "馬乗り": "骑乘",
    ...
    "オナホ": "性玩具", "オナホコキ": "性玩具", "ローター責め": "性玩具", "ペニスポンプ": "性玩具",
    "コックリング": "性玩具", "アナルプラグ": "性玩具", "アナルしっぽ": "性玩具",
    # `くぱぁ` 是把阴部掰开给镜头看，说的是镜头怎么给，和 `局部アップ` 同一格。
    "くぱぁ": "局部特写",
    ...
    "Nice Pussy": "美穴", "Beautiful Pussy": "美穴", "美マン": "美穴", "スジマン": "美穴",
    "Big Cock": "巨根", "Big Dick": "巨根", "巨根": "巨根", "デカチン": "巨根", "巨ちん": "巨根",
    ...
    "敏感": "敏感体质",
    "Shaved Pussy": "白虎", "Shaved": "白虎", "パイパン": "白虎", "ぱいぱん": "白虎",
    "猫耳": "兽耳兽装",
    ...
    "セーラー": "制服",
    # `スーツ` 在来源里配的是 OL 那一身（`OLスーツ`），和角色那一格 `秘书OL` 分开记穿着。
    "スーツ": "OL制服",
    "Lingerie": "情趣内衣", "Underwear": "情趣内衣", "ランジェリー": "情趣内衣",
    "下着": "情趣内衣", "猫ランジェリー": "情趣内衣", "コルセット": "情趣内衣",
    # `ボンテージ` 是皮革与漆皮的束身装，`エナメル` 说的是同一种漆皮料子。
    "ボンテージ": "皮衣皮裙", "エナメル": "皮衣皮裙",
    "Swimsuit": "泳装", "Swimsuits": "泳装", "School Swimsuits": "泳装",
    "Bikini": "泳装", "水着": "泳装", "スクール水着": "泳装", "ビキニ": "泳装",
    "競泳水着": "泳装",
    "Leotards": "体操服", "Gym Clothes": "体操服", "ブルマ": "体操服",
    "体操着・ブルマ": "体操服", "レオタード": "体操服", "体操着": "体操服", "体操服": "体操服",
    "Bunny Girl": "兔女郎", "バニーガール": "兔女郎", "バニー": "兔女郎",
    ...
    "Role Play": "角色扮演",
    # 扮成什么的那几格：`コス` 是 `コスプレ` 的省写，`アニコス` 是动漫角色的扮装。
    "コス": "角色扮演", "コスチューム": "角色扮演", "アニコス": "角色扮演",
    "サキュバス": "角色扮演", "可愛いサンタ": "角色扮演", "レースクイーン": "角色扮演",
    "Maid": "女仆", "メイド": "女仆", "牛メイド": "女仆",
    ...
    "College Girl": "学生", "女子校生": "学生", "女学生": "学生", "学生": "学生",
    "ロリ": "萝莉",
    ...
    "アイドル": "偶像艺人", "芸能人": "偶像艺人",
    # 作品上的 `グラドル` 说的是出镜的是写真偶像；资料页上同一个词说她的行业，见 `PROFILE_GENRES`。
    "グラドル": "偶像艺人",
    ...
    # FC2 标题开头的 `【無】`、`【モ無し】` 是無修正的省写，卖家原样搬进标签。
    "無": "无码", "モ無し": "无码",
...
PROFILE_GENRES: dict[str, tuple[str, ...]] = {
    ...
    **dict.fromkeys((
        ...
        "極稀にAV女優", "改名・移籍", "改名・再移籍", "改名", "移籍", "移籍(改名なし)", "移籍→改名",
    ...
    **dict.fromkeys((
        ...
        "美体", "パーフェクトボディ", "パーフェクトボディー", "スーパーボディ", "最強ボディ",
...
NON_CONTENT_GENRES: frozenset[str] = frozenset({
    ...
    "カラコン", "ブリーフ", "縞しま", "ボディコン", "ロングヘアー", "股間に穴", "全裸", "ヌード",
    "着衣", "完全着衣", "着衣のSEX", "ゴム", "Wピース", "ピース", "着替え", "サランラップ",
    "トイレ", "ベッド", "試着室", "プリクラ", "引っ越し", "新居", "クリスマス",
    # FC2 卖家自填的发行说法：谁拍的、公开过没有、卖几份、片子多长，和 `Independent`、
    # `独占配信`、`4時間以上作品` 同一条线。`0930` 是厂牌 `エッチな0930` 的简写。
    "個人撮影", "個撮", "オリジナル", "未公開", "限定", "独占", "VIP", "無編集", "長編",
    "JAPANESE", "0930", "むすめガチャ", "有名人", "女優", "モデル",
    # 泛指做爱本身，和 `All Sex` 一样每部都挂得上。
    "セックス", "SEX", "性交", "エッチ", "ハメ", "2回戦",
    # 夸法、氛围与高潮的说法，和 `Sexy`、`Orgasm` 同档；最后几个是从标题截下来的剧情短语。
    "セクシー", "極上", "凄い", "気持ちいい", "超気持ちいい", "えろい", "エロ過ぎ", "ドスケベ",
    "恥ずかしい", "おしゃれ", "従順", "ご奉仕", "ご奉仕プレイ", "イキまくり", "子宮イキ", "腹イキ",
    "コンビネーション", "グリグリ", "テカテカ", "指一本", "擬似", "勝負", "我慢できない",
    "極楽天国", "征服感満載", "聖夜に一発", "早漏改善", "チンポ検査",
    # 人名、昵称与作品名说的是谁、扮谁；扮演这件事由 `コスプレ` 那一格说。
    "しょうなちゃん", "あいちゃん", "猫ちゃん", "KOF", "不〇火舞", "テ〇ファ", "FG○",
})
