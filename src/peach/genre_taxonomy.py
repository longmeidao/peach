...
CONTENT_GENRES: dict[str, str] = {
    ...
    "口交": "口交",
    "Deep Throat": "深喉", "イラマチオ": "深喉", "ディープスロート": "深喉",
    ...
    "Doggystyle": "后入", "Strong Doggy Style": "后入", "バック": "后入",
    ...
    "Cuckold": "绿帽NTR", "NTR": "绿帽NTR", "寝取り・寝取られ・NTR": "绿帽NTR",
    "Incest": "近亲", "近親相姦": "近亲",
...
def is_non_content_genre(raw: object) -> bool:
    return normalise_genre(raw) in _NON_CONTENT_INDEX
...
def map_genres(genres: Iterable[object]) -> tuple[list[str], list[str]]:
    ...
    for genre in genres or []:
        ...
        if not key or key in _NON_CONTENT_INDEX:
            continue
