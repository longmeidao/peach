...
CONTENT_GENRES: dict[str, str] = {
    ...
    "Squirting": "潮吹", "Squirts": "潮吹", "Squirt": "潮吹", "潮吹き": "潮吹", "潮吹き娘": "潮吹",
    "母乳": "母乳", "Lactation": "母乳", "スカトロ": "排泄", "Scat": "排泄",
    "フィストファック": "拳交", "フィスト": "拳交", "Fisting": "拳交",
    "W姦": "3P多人", "生姦": "中出内射", "高速騎乗位": "骑乘",
    "オナホール": "性玩具", "ギャプ萌え": "反差",
    ...
    "貧乳": "贫乳", "貧乳・微乳": "贫乳", "微乳": "贫乳", "ちっぱい": "贫乳",
    # 乳型一种一个标签：来源说到乳晕、乳头还是形状，就取到那一级。
    "美巨乳": "美巨乳", "天然美巨乳": "美巨乳", "美爆乳": "美爆乳", "美貧乳": "美贫乳",
    "ガリボイン": "瘦巨乳", "ロケット乳": "火箭乳", "尖がりオッパイ": "尖乳",
    "垂れ乳": "下垂乳", "垂れ長乳": "长垂乳", "エロ乳房": "色气乳房",
    "美乳輪": "美乳晕", "美人輪": "美乳晕", "巨乳輪": "大乳晕", "巨大乳輪": "大乳晕",
    "デカ乳輪": "大乳晕", "パフィーニップル": "凸乳晕",
    "エロ乳首": "色气乳头", "デカ乳首": "大乳头", "美乳首": "美乳头",
    "陥没乳首": "凹陷乳头", "陥没チクビ": "凹陷乳头", "勃起乳首": "勃起乳头",
    "敏感乳首": "敏感乳头", "黒乳首": "深色乳头", "長乳首": "长乳头",
    "偽乳": "隆胸", "偽乳(豊胸乳)": "隆胸", "豊胸": "隆胸", "豊胸乳": "隆胸",
    "Ass Lover": "美臀", "Big Asses": "美臀", "Big Ass": "美臀", "Butt": "美臀", "Nice Ass": "美臀",
    "美尻": "美臀", "尻": "美臀", "お尻": "美臀", "尻フェチ": "美臀",
    # `巨尻` 说的是大，`美尻` 说的是好看，和 `巨乳`／`美乳` 一样分两格。
    "巨尻": "巨臀", "美巨尻": "美巨臀",
    ...
    # `美少女`、`美女`、`美人` 说的都是好看，没有分得开的区别，归一个「高颜值」；
    # `清楚` 说的是清纯干净的气质，`かわいい` 说的是可爱，这两种和好看不是一回事。
    # 词尾的 `系` 只说「这一类」，和本词同去向。`Neat and Clean` 是 r18 对 `清楚` 的英译。
    "Beautiful Girl": "高颜值", "Beauty": "高颜值",
    "美少女": "高颜值", "美少女系": "高颜值", "美女": "高颜值", "美人": "高颜值",
    "美人名女優": "高颜值", "美顔": "高颜值", "Pretty Face": "高颜值", "Pretty Girl": "高颜值",
    "清楚": "清纯", "清楚系": "清纯", "清純": "清纯", "清純系": "清纯", "Neat and Clean": "清纯",
    "可愛い": "可爱", "かわいい": "可爱", "可愛い系": "可爱", "かわいい系": "可爱", "Cute": "可爱",
    "剛毛": "多毛", "天然陰毛": "多毛", "陰毛あり": "多毛", "未処理マ○毛": "多毛", "尻毛": "多毛",
    "色白": "白皙", "美白": "白皙", "白い肌": "白皙",
    "くびれ": "细腰", "クビレ": "细腰",
    "ダイナマイトボディ": "火辣身材", "ダイナマイトボディー": "火辣身材", "エロボディ": "色气身材",
    # `ぽっちゃり` 说肉感（丰满），`ぽちゃ`／`ややぽちゃ` 说的是有点胖。
    "ぽちゃ": "微胖", "ややポチャ": "微胖", "ややぽちゃ": "微胖",
    "軟体": "身体柔软", "腰が柔らかい": "身体柔软", "Y字バランス": "身体柔软",
    "ハーフ": "混血", "タトゥー": "纹身", "刺青": "纹身",
    "ビラマン": "大阴唇", "デカビラ": "大阴唇", "グロマン": "深色阴部", "名器": "名器",
    "アニメ声": "动漫音", "可愛い声": "娃娃音", "腹筋": "腹肌", "黒髪": "黑发",
    "敏感": "敏感体质",
    ...
    # 单说时讲的是女方，男方那一边站上一律写成 `Virgin Boy`。女方那一格是出道卖点或剧情，
    # 核实不了，所以叫「处女设定」。
    "童貞": "处男", "Virgin Boy": "处男",
    "処女": "处女设定", "Virgin": "处女设定", "Virgin Girl": "处女设定",
    "美熟女": "美熟女", "微熟女": "微熟女", "美魔女": "美魔女",
    "変態": "变态", "ニューハーフ": "跨性别", "Transsexual": "跨性别",
    "ギャル系": "辣妹",
...
#: 女优资料页（minnano-av）标签的专用去向，查表排在 `CONTENT_GENRES` 前面。
#:
#: 同一个词在作品上和资料上说的不是一件事：作品上的 `風俗嬢` 是场景「风俗店」、`看護師`
#: 是角色扮演「护士」，资料上说的是她本人做过的行业，所以行业词只进这张表。
#: 资料上还有几样作品上没有的写法：一格写了几个标签的（值是几个）、带箭头说她风格
#: 前后变了的（整条保留）、厂牌与改名移籍这些经历（空元组，不作为标签）。
PROFILE_GENRES: dict[str, tuple[str, ...]] = {
    # —— 风俗，一种一个 ——
    **dict.fromkeys(("デリヘル嬢", "現役デリヘル嬢", "現役デリ嬢", "元・デリヘル嬢"), ("上门服务女郎",)),
    **dict.fromkeys(("ソープ嬢", "現役ソープ嬢"), ("泡泡浴女郎",)),
    "セクシーパブ嬢": ("性感酒吧女郎",),
    # 站上只说做过风俗，没说哪一种。
    **dict.fromkeys(("風俗嬢", "風俗歴有り"), ("风俗从业",)),
    # —— 其余行业，各译各的；「元」「現役」只说时间，行业是同一个 ——
    **dict.fromkeys(("キャバクラ嬢", "現役キャバ嬢", "現役キャバ嬢(六本木・RedDragon)"), ("陪酒女郎",)),
    "現役ホステス.": ("女公关",),
    # `踊り子` 在日本就指脱衣舞台上的舞者。
    **dict.fromkeys(("ストリッパー", "現役ストリッパー", "踊り子", "ストリップデビュー予定"), ("脱衣舞娘",)),
    "撮影会ヌードモデル": ("裸体模特",), "SMモデル": ("SM模特",), "コスプレイヤー": ("Coser",),
    **dict.fromkeys(("グラドル", "元グラドル", "グラビアアイドル"), ("写真偶像",)),
    "元アイドル": ("偶像",), "タレント": ("艺人",), "元レースクイーン": ("赛车女郎",),
    "シンガー": ("歌手",), "YouTuber": ("YouTuber",), "映画出演": ("电影演员",),
    "ピンク映画女優": ("粉红电影演员",), "監督": ("导演",), "元アスリート(新体操)": ("运动员",),
    # 作品上的「护士」是角色扮演，这里是本职。
    "現役看護師": ("现役护士",),
    "SOD女子社員新卒採用内定者": ("SOD女员工",), "金券業者": ("金券商",), "蟹漁師": ("捕蟹渔民",),
    # —— 出身、经历与特长 ——
    "クォーター": ("四分之一混血",), "アメリカ系": ("美国血统",), "韓国系": ("韩国血统",),
    "台湾系": ("台湾血统",), "タイ系": ("泰国血统",), "イタリア系": ("意大利血统",),
    "カナダ系": ("加拿大血统",), "スペイン系": ("西班牙血统",),
    "整形": ("整形",), "顔整形": ("整形",), "口元にホクロ": ("嘴角痣",), "美爪": ("美甲",),
    "NGナシ": ("无NG",), "新体操": ("艺术体操",), "バレエ": ("芭蕾",),
    # 说的是出道作的卖点，不是她现在的状态，不并进「处女设定」。
    "処女デビュー": ("处女出道",),
    # 着衣的色情影像，AV 之前的出身。
    "着エロ": ("擦边影像",),
    ".美人": ("高颜值",), "清楚お嬢様系": ("清纯", "千金小姐"),
    **dict.fromkeys(("豊胸乳(89F)", "豊胸後(響つかさ)", "豊胸後(あきばもも)", "豊胸後改名"), ("隆胸",)),
    "全身タトゥー(改名後)": ("纹身",),
    **dict.fromkeys(("中出しOK",), ("中出内射",)), "無修正作品あり": ("无码",),
    **dict.fromkeys(("東熱大乱交", "東熱大乱交(便器女)"), ("多人",)),
    # —— 风格转变：箭头两边都留 ——
    "ロング→ショートカット": ("长发→短发",), "ロリ→ギャル": ("萝莉→辣妹",),
    "ロリ→黒ギャル": ("萝莉→黑辣妹",), "ロリ→熟女": ("萝莉→熟女",), "ロリ系→熟女": ("萝莉→熟女",),
    "美少女→熟女(ムッチリ)": ("美少女→丰满熟女",), "微乳→巨乳": ("贫乳→巨乳",),
    "微乳→偽乳(豊胸乳)": ("贫乳→隆胸",),
    # —— 站上把几个标签写进了一格 ——
    "ショートカット、美乳、レズ": ("短发", "美乳", "百合"), "美乳美尻": ("美乳", "美臀"),
    "巨尻ごっくん": ("巨臀", "吞精"), "無修正生中出し": ("无码", "中出内射"),
    "無修正流出": ("无码", "泄密流出"), "偽乳(豊胸乳)エロ乳首": ("隆胸", "色气乳头"),
    "美巨乳(豊胸乳)": ("美巨乳", "隆胸"),
    # 「最強ボディ」是夸法，不单独成标签。
    "最強ボディ巨乳": ("巨乳",), "最強ボディエロ乳首": ("色气乳头",),
    # —— 不作为标签 ——
    # 厂牌、站点与系列：去向是作品的厂牌，不是她的标签。
    **dict.fromkeys((
        "FC2", "カリビアン", "カリビアンコム", "東京熱", "一本道", "heyzo", "Hey動画", "天然むすめ",
        "パコパコママ", "ガチん娘", "CovertJapan", "エッチな4610", "エッチな0930", "GirlsDelta",
        "ガールズデルタ", "tenshigao", "しろハメ", "麻豆传媒", "S1", "人妻斬り",
        "撮りおろし徹底屈辱ビデオ", "問答無用姦", "鬼逝", "餌食牝", "一刀両断", "1000人斬り",
        "太平洋娘", "洋物", "単体"), ()),
    # 引退、复出与改名移籍：生涯那一格已经说了还拍不拍；这里的箭头说的是经历先后，不是风格。
    **dict.fromkeys((
        "引退", "AV女優引退", "AV引退", "2008年引退", "2012年復帰", "復活", "故人", "伝説", "名女優",
        "女優", "極稀にAV女優", "改名・移籍", "改名・再移籍", "改名", "移籍", "移籍(改名なし)", "移籍→改名",
        "引退→復活(改名・移籍)", "改名・移籍→改名(移籍なし)", "改名・移籍→再改名・再移籍",
        "改名(移籍なし)→再改名・移籍", "移籍(MarksGroup→LINX)→改名・移籍", "改名(移籍なし)→改名・移籍",
        "引退→復活→引退→再復活", "引退→復活・移籍(改名なし)", "身バレ"), ()),
    # 所属团体、夸法、学历出身、数值与编者注记、政治立场：都不说内容。
    **dict.fromkeys((
        "恵比寿マスカッツ", "S3チーム「cuteen」メンバー", "”millionガールズZ”", "鈴木心4姉妹",
        "セクシー", "美体", "パーフェクトボディ", "パーフェクトボディー", "スーパーボディ", "最強ボディ",
        "絶叫", "可愛い笑顔", "可愛いしぐさ", "中卒", "帰国子女", "スイス出身→ロシア出身", "ロス出身",
        ".", "H93？", "48kg", "44.4kg", "推定体重40kg未満", "E70", "ブラサイズH65",
        "「瑞樹ララ」は別人", "「天然むすめ」の「南ゆき」は別人", "中山こころ", "妊娠線(葵百合香)",
        "乳論", "親中", "反日", "親韓"), ()),
}
...
_PROFILE_INDEX = {normalise_genre(key): value for key, value in PROFILE_GENRES.items()}


def resolve_profile_tag(raw: object,
                        decisions: Mapping[str, str | None] | None = None) -> tuple[str, ...] | None:
    """资料页一个标签的去向：几个 Peach 标签、空元组（不作为标签）、或 `None`（未收录）。

    用户的决定仍排第一；其次是资料专用的 `PROFILE_GENRES`，最后才落回作品那张表。
    """
    key = normalise_genre(raw)
    if not key:
        return ()
    if decisions and key in decisions:
        return (decisions[key],) if decisions[key] else ()
    if key in _PROFILE_INDEX:
        return _PROFILE_INDEX[key]
    tag = resolve_genre(raw)
    if tag == UNMAPPED:
        return None
