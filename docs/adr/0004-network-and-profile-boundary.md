...
- 局域网服务由 FastAPI 生命周期发布 `peach.local` mDNS；可用 `--no-mdns` 关闭，不再依赖旧 HTTP server。Windows 使用系统 `DnsServiceRegister` DNS-SD 后端，避免 Python zeroconf 与 CrossPaste/PikPak/Steam 等多个 UDP 5353 使用者竞争；其他平台保留 zeroconf 后端。
