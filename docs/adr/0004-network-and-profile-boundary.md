...
- 局域网服务由 FastAPI 生命周期发布唯一入口 `peach.local`；可用 `--no-mdns` 关闭，不再依赖旧 HTTP server。所有平台统一使用 Python zeroconf，并在 Windows 保留迁移前已实际成功的全合格网卡监听语义。不得只凭 UDP 5353 监听者数量推断冲突，也不得用 DNS-SD 注册回调替代客户端解析验证。详细复盘见 ADR-0009。
