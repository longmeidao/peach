...
- 全新机器只跑 `peach init`。不带参数且 stdin 是终端时进问答（题目、默认值与写文件逻辑在
  ...
  配置页是 `/configuration`，从管理菜单的「配置」或设置弹层「这台电脑」摘要卡上的
  「打开配置页」进，按「通用 / 媒体 / 网络与访问 / 更新与维护」分开列。只在运行 Peach 的
  这台电脑上、且由托盘管理的服务才可改，别的设备的管理菜单不列它，设置里那一格写的是要去
  哪里改；数据走 `/api/configuration`，保存后写重载标记由托盘接手。媒体修复在数据管理页，
  订阅源在关注管理页的「订阅源」页签，分工判据见 ADR-0050。
...
- 需要设置时托盘不构建正常规格（全新机器上 `build_service_specs()` 会因为缺 TLS 材料抛
  ...
  成功页的普通入口带 `/?onboarding=1`，进入 Peach 后在右下角浮窗显示安装教程；选择浏览器历史
  导入则先去 `/taste?onboarding=1`。待办标记只存在当前浏览器，教程在不同页面间持续显示，并通过专用状态接口读取馆藏、
  采集凭证、浏览器历史、关注、关注凭证和复核的实际状态；单项可跳过并短时撤销，全部完成或
  跳过后教程自动消失。
- 切换由 `tray.SetupGate` 做，Windows 托盘与 macOS 菜单栏共用。它挂在健康轮询里（Windows 10 秒、
  macOS 5 秒），每轮新鲜 `settings_file.load_config()`：不再需要设置就停掉引导服务，按新数据根
  构建正常规格并启动，托盘进程本身不重启。独立测试包按设置在高位端口提供 HTTP：本机模式绑定
  `127.0.0.1` 且不发布 mDNS，局域网模式绑定 `0.0.0.0` 并发布 `<mdns_name>.local:<端口>`；托盘打开与
  健康检查始终走回环。源码部署仍由 `build_service_specs(tls_dir=..., mdns_hostname=...)` 构建 HTTP
  跳转与 HTTPS 入口，TLS 材料没齐就原地等下一轮。mDNS 名由这里传进规格，不读 import 期旧值。
- 首扫不跑在引导服务里，因为那个进程在切换的那一刻就被停掉了。表单勾了「现在扫描」只写一个
...
- `peach scan <来源ID> [根目录]` 把一个目录的文件元数据 upsert 进账本，只新增行与刷新
  ...
  （macOS 上是挂载点）之内，否则拒绝，因为写进去的行翻译不回本机路径。
- 已经在跑的机器用 `peach init --from-existing`：只写设置文件，不建库、不动 `peach-data/`
  下任何现有文件。它把当前实际生效的配置原样写进设置文件，猜不出来的坐标（局域网 writer 地址、
...
- 第一阶段写下的 `[media] R = '...'` 是盘符键，第二阶段起是硬错误：`serve` 会拒绝启动
  ...
  它写出的是**内建默认加环境变量**，不继承那个读不出来的旧文件；命令会先把这件事打印
...
- 发布入口是 `scripts/build_windows.ps1`：先用 `scripts/generate_brand_assets.py` 生成方形 Logo 与多尺寸 `.ico`，再构建单一 `dist/Peach/Peach.exe`；无参数运行托盘，`serve`／`migrate` 运行 CLI。桌面快捷方式和开机自启都在配置页的「开机自启」一组里开关，由 `src/peach/desktop_startup.py` 写入；`scripts/manage_tray_startup.ps1` 只作发布包的排障入口。
...
- 命名隧道（Named Tunnel）是源码开发环境专用的第二种形态，地址固定、重启不变。独立包读到 `tunnel.mode = "named"` 会拒绝启动隧道并报「独立包只支持临时链接」，配置页在那里也不渲染这一块。四步：
  1. 在 Cloudflare Zero Trust 的 Networks → Tunnels 里新建一条 Cloudflared 隧道，拿到它的隧道令牌（Token）。
...
- 生成产物走 Syncthing 单向同步，和账本、和 Git 是三条互不替代的通道：Windows send-only、Mac receive-only，五个文件夹 `snapshots`／`posters`／`avatars`／`logos`／`covers`，Mac 侧根目录在 `peach-data/artifacts/`（不要把指向它的符号链接设成同步目录），Trash Can 版本保留 30 天。
...
- `--bump` 只写文件不提交，并把定版那一节原样打印出来供人过目。
...
- 「检查更新」只 fetch 和比较；「同步开发进度」只做 `merge --ff-only`，不 stash、不 rebase、不 `--force`，因为并行工作树和主检出共用同一个对象库与 reflog，任何改写历史的「顺手解决」都会把别的分支一起拖下去。工作区脏或两边分叉时原样报出来交给人。本地不落后远端、或者根本连不上远端时，它转为按构建身份判断打包托盘要不要重建（见「桌面入口与发布」）；那条路径不拦脏工作区，因为构建跑的就是检出里的这一份代码。
...
- 登录页勾选「保持登录」保持 30 天，不勾只保持本次浏览器会话（最长 12 小时）；有效期由服务端签名并检查，页面刷新不续期。API 自动化继续使用独立 `X-Token` 凭据，修改访问密码不撤销机器凭据；机器凭据轮换仍走 `peach token --rotate` 并重启服务。
...
- 网盘目录由人在 CloudDrive 外部整理后，必须经「管理 → 资源同步」显式对账，不做后台静默删除：核对已配置的本地与网盘来源，离线来源整源跳过，单个目录不可读时保留账本行并回报。
...
- `generated/avatar-thumbs` 是实体图缩到长边 640 的索引页派生件，每台机器自己现缩，不进 Syncthing、不进备份；整个目录删掉只会让下一次打开索引页慢一屏，缺了它页面退回原件，显示不受影响。装进一批新头像后用 `scripts/build_entity_thumbs.py` 提前缩好，把「一屏一百来张各现缩一次、每张约 68 ms」挪到没人等的时候。
...
- 导入运维脚本不得触发文件、网络或数据库副作用；`scrape_codes.py` 默认写可续跑复核 CSV，`clean_names.py` 先预览，`--apply` 必须同时给出 `--backup <路径>`，备份写完后当场校验完整性（`peach.scripting.open_for_write`）。
...
- FC2CMADB 的文章存档只作官方商品页镜像证据：标题与保守翻译后的标签写候选 CSV 并与最新 JAV 元数据批次一起进入 `/review`，不自动写 ledger；封面把列表 `w276` 换成实测存在的 `w1200`，再走 `fetch_jav_covers.py --fc2-only` 的解码、宽度、磁盘和原子写入门槛，失败留逐条日志。
