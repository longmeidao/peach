...
- **Logo 文件一律是不透明方图。** 小位铺满（cover）；大位按 contain 摆，图不足框的八成时不再放大，
  ...
  12%；内容落在内切圆之外的比例超过 `PLATE_CIRCLE_LOSS`（0.025）把画布补到内容的外接圆；整张短边
  不到小圆片要的实像素（`PLATE_MIN_SIDE`，64 = 32 CSS px 在 2 倍屏上的实像素）就用自己的底色补到
  那个数，浏览器不必再放大它。补边的上限卡在 `PLATE_MIN_SPAN`：再往外撑，内容占宽就掉到 0.6 以下，
  成了第一条要裁的那种大留白，两条规则会在同一张图上来回拉锯，重跑一遍就不再幂等。2026-09-08
  实测命中 7 张：DorcelClub 57→64、Flower 三张 55→64、LINX 63→64；HEYZO.icon 39→50、
  Prestige.icon 42→53 是撞上这个上限停下的。像素一律
