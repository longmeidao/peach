...
前面——javdb 按出口 IP 计配额、主机间隔 5 秒，先问便宜的两家不影响结算，只影响谁先撞限流。
...
- 镜像也没有的落到 JavArchive。它的作品地址里夹着站内文章号和标题（`/926949-FC2-PPV-4137487-…-pn.html`），
  拼不出来，所以先问 `/search?q=<番号>` 再取那一条；商品号按数字边界比，`4137487` 不能命中 `41374870`。
  这一档只给标题和一张转存封面，没有販売日、卖家和商品标签，正文那几段是转载来的下载链接，一概不取。
  封面在 `img.javstore.net` 上，沿用 FC2 自己的 `pl`（大）/ `ps`（小）命名，`_s.jpg` 那种是多帧拼成的
  长条预览，不是封面。2026-09-22 实测 `FC2-PPV-4137487` 在 fc2cmadb 是 404、这里有，封面 709×399——
  比官方原图差一档，所以排在最后，只在前两处都说没有时才走到。`robots.txt` 全站放行。
