...
- 下架的商品页仍回 200 且不再带 Product，这不是抓取失败。接着问镜像站
  ...
  站方用户维护，按社区来源登记，取值进复核。它按 IP 限流，没登录连着问几页就一路 429 并把整个
  来源冷却掉，所以要在「来源和凭证」里给它配一份登录 cookie。
- 镜像也没有的落到 JavArchive。它的作品地址里夹着站内文章号和标题（`/926949-FC2-PPV-4137487-…-pn.html`），
  ...
  原图差一档，所以排在最后。`robots.txt` 全站放行。

链上哪一档给出封面地址才停，不是哪一档答上就停：前面那两处常常有资料而没有图。停在它那里，
后面真有图的那一档就再也没机会（2026-09-22 实测 `FC2-PPV-3232110` 从 fc2cmadb 拿到的地址是
404，JavArchive 上有一张）。前面答过的资料照旧带着走，多一档就多一批标签和一个图源（ADR-0030）。
