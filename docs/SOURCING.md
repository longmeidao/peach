...
- 下架的商品页仍回 200 且不再带 Product，这不是抓取失败。接着问镜像站
  ...
  站方用户维护，按社区来源登记，取值进复核。游客就能读，不收 Cookie。它按出口 IP 限流，撞上
  429 整个来源冷却，换出口节点比带登录态管用（2026-09-23 实测香港出口 429、日本出口 200）。
...
- 收 Cookie 的来源（JavDB、JavBus、Instagram）其粘贴与 Netscape 文件导入复用 CredentialStore；
