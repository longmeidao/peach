...
两条轴不要混。最直观的例子是 javbus：结算上它是兜底（ADR-0035），查询上却排在 javdb
前面——javdb 按出口 IP 计配额、主机间隔单独设（见下文），先问便宜的两家不影响结算，只影响谁先撞限流。
...
那一档整档一起问，不逐家短路——免复核要两家取值一致（ADR-0030、ADR-0034），封面互证要两个
...
- 镜像也没有的落到 JavArchive。它的作品地址里夹着站内文章号和标题（`/926949-FC2-PPV-4137487-…-pn.html`），
  拼不出来，所以从 `/search?q=<番号>` 进；商品号按数字边界比，`4137487` 不能命中 `41374870`。
  搜索结果那一条自己就带齐这一档能给的东西：`title` 属性是完整标题（地址里那份把空格换成了连字符，
  所以标题只认属性），`img` 的 `…ps.jpg` 换名就是大图 `…pl.jpg`。够用就不再取作品页（`parse_search`），
  只有那条没有缩略图时才走第二跳。2026-09-22 实测 4 个番号里 2 个一跳拿到（`4137487` 255×294→709×399、
  `835964` 510×690→1417×825，各 3～4 秒），`2110084` 回退到作品页，那部站上本来就没有图。
  这一档只给标题和一张转存封面，没有販売日、卖家和商品标签，正文那几段是转载来的下载链接，一概不取。
  封面在 `img.javstore.net` 上，沿用 FC2 自己的 `pl`（大）/ `ps`（小）命名，`_s.jpg` 那种是多帧拼成的
  长条预览，不是封面。`FC2-PPV-4137487` 在 fc2cmadb 是 404、这里有，封面比官方原图差一档，所以排在
  最后，只在前两处都说没有时才走到。`robots.txt` 全站放行。
...
- javbus 是兜底来源（`metadata_policy.FALLBACK_SOURCES`）：同一个字段上还有别家可用时它的取值不当证据，
...
- LIGHT 于 2026-09-05 实测已拆成两家：`ELTRA(エルトラ)旧・LIGHT`（production=1251，10 位）与
  ...
  LIGHT 的旧账号 `light_promg`，这条链路本身就是拆分的旁证。
...
- **头像目录是几条管线共用的，`--apply` 只往好里换。** `--force` 的语义是「已有头像也参加竞选」，
  ...
  640×960 半身照被换成 JBS-023 的双联封面。现在落盘前先按同一把 `rank_key` 量一次盘上那张，脸没有
...
- **换掉已装的图要逐张比过，还要能被收进目标集。** `harvest_targets` 按「有没有 `<safe>.img`」
  ...
  装完边车就一致，下一轮自己退出。落盘那道「只认更大的」守卫对指定来源不设（`PINNED_KINDS`）：
...
- **合并之后标识要跟着改挂。** 标识按 `logo_key(canonical_name)` 落盘，被丢弃那一侧手上的方标在合并
...
- **一条官网都没有的公司走社媒兜底**。`ICON_LINK_KINDS` 之外还有 `FALLBACK_LINK_KIND="social"`，只在这家
