...
32. **系统清理与瘦身**（用户 2026-09-11 提出）：一个统一入口，先按类别列出各自占多少、删掉会失去什么，再由用户挑着清。可清的有头像与 Logo 的候选缓存（`provider-cache/` 下的 `objects/`、`requests/`）、抽帧与九宫格、HLS 分片与转码产物、复核 CSV 与标注图、日志与 state 快照。分档判据是「删了之后要重做一遍什么」：请求快照删了只是下次重取，`evidence/` 是「这个人用过哪些图」的历史、删了换头像就再也找不回旧图，两者不能同档。账本、真实媒体、备份和 `avatars-superseded/` 不在清理范围内。
    起因是换头像功能把取到的每一张图都留在候选缓存里，盘上只增不减；但入口本身要按上面整张清单做，不是只清头像。

合计：**39 项开放需求**，其中 7 项已有骨架，32 项尚未实现。已完成的需求不在这里留痕，去 Git 历史查。
...
42. 先定大图容器比例，再决定剩下 864 张封面要不要生成 `poster_box` 边车。2026-09-12 只读盘点 1014 张封面：折痕 577、右半居中 106、不裁 331。把框换算成大图版式的横向 `object-position` 之后，683 张有框的里 611 张夹到 100%，与接线之前写死的取值逐像素相同；真会移动的只有 62 张，位移中位数 0 个百分点、最大 3.79 个百分点（约 47 源图像素）。根因是大图容器 `COVER_FRONT_RATIO = 0.7` 比 2:3 的取景框略宽，框整个落在可见窗口里还有富余，而框又贴着封套右缘，锚点于是恒被夹到最右。要让折痕真正说了算，得把容器改成正好 2:3（卡片高度涨约 5%），那是一次版式决策；不改则等于确认「贴封套右缘」这条规则对这批封面成立。本机已按 `--limit 150` 生成边车（不裁 106、折痕 38、右半居中 6）。
43. 逐对判定 `115` 来源的 11 对路径大小写重复：2026-09-12 只读盘点，成因与 `local` 那次不同，是 `.MP4` 与 `.mp4`、`MIDE-950-C` 与 `mide-950-C` 这类扩展名与目录名的大小写，且多数两侧都挂着标签与快照（例如 `86263` 有 5 条标签、`28608` 有 6 条）。不能像 `local` 那样机械地保旧删新，要一对一看哪侧的标签与快照更全，合并后再删另一侧。
