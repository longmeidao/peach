...
3. **在线追更**：kemono/coomer/pawchive、rule34video、rule34.xxx、f95zone 五类连接器、WIP/alt/跨站重复判定、`follow_source`/`follow_item`、`peach follow`、看的 `/follow` 与管的 `/follow-manage` 两个页面（粘链接或只给名字/id，自动跨六个来源查找）已上线；writer 已用 APScheduler 按设置自动轮询。仍缺媒体下载落地，以及 simpcity 的可用入口（DDoS-Guard，未取得）。
