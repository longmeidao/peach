...
正式工作树已实现共享视觉层、管理与详情卡片、设置布局、数值输入和旧版隔离；18984 直接读取该工作树用于只读验收。生产尚未切换。Remix Icon 候选审查在预览 `/icon-review.html`，现有已选图标保留至用户筛选。迁移不包含版本号或其他分支发布工作。

统计与口味已接入 Stat Cards、独立实现的排名条、雷达与分布环；侧栏使用同一导航的 60/260px 宽窄态，手机从左侧滑入。设置滚动渐隐、手机输入字号、焦点环留白及登录保持时长已接入。业务进度的适配范围与不适用图表见 [Board 适配](BOARD_UI.md)。

侧栏底部固定设置和明暗开关，顶部独立按钮控制展开；媒体库图标负责打开选择面板，首页入口使用 Peach logo。路径可命名分组并选择图标，网盘默认图标由本机提供；库选择限定作品列表与筛选项。统计、口味和维护任务保持全局。关注提供宽松与紧凑排版，共享作者分组、排序、多选和批量删除；宽松模式可收起作者，最近观看标题可跳转视频。详情优先响应 Esc，手机排序栏可横向滚动。预览不保存真实配置。
