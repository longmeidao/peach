...
24. 复核 `studio-icons-mgstage-<日期>.csv` 的 12 家后另行授权跑 `harvest_studio_icons.py --install`：全部 `字标补白` + `ok`，来源是 MGStage 名录的官方字标，这 12 家账本里一张图都没有。
25. 合并按罗马字名与日文名各存一份的重复厂牌实体（`Prestige` 5630 / `プレステージ` 8610、`SOD Create` / `SODクリエイト`、`MOODYZ` / `ムーディーズ` 等），资产数与标识现在各挂一半。走 `merge_entity`，先备份、先取授权。
26. 给账本厂牌补日文别名。MGStage 名录 351 家只对上 29 家，卡点是账本 129 个厂牌只有 16 条别名、几乎没有日文名——补完别名再对一次，覆盖面会一次性抬上去。
27. 其他厂牌官网的厂标与演员资料广度扫描（SOD、FALENO、Attackers、S1、Moodyz 等），用户 2026-09-04 定为「先做 b 看效果」之后的下一轮。
28. 用 javtiful 的 `/ja/actress/<slug>` 补演员的罗马字↔日文配对：315 页约 7560 位，切语言前缀就出日文名。厂牌名不随语言切换，这条只服务演员别名。
