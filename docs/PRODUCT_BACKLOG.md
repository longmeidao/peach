...
27. AKA-024 的系列同时挂在 `クロロホルムレ●プ`（#8416）与 `Night Safari`（#8636）两条实体上，且前者带一条 `r18dev:series-localization` 写的 `Night Safari` 别名。AKA-022 才是 クロロホルムレ●プ；先判本地化那一步为什么把两个系列对到一起。
28. 95 位演员的规范名是假名、一条中文别名都没有（`深田えいみ`、`河合あすな`、`長谷川るい` 这些都在内），按中文名搜索一律落空。`localize_performer_names.py` 靠的 avdb 映射 XML 不在这台机器上，且它对假名名字本来就只能等收录。现成证据是 javdb 资料页：124 页缓存里 50 页有名字块、33 页中日两种写法都有，`javdb_names()` 已经能取。缺的是把同一页上多个新旧艺名配成对（`橋本有菜` 那页列了三个日文名三个中文名），以及繁转简（javdb 是繁体站，opencc 已在依赖里）。
