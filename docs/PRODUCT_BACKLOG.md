...
42. 图库同名多张、却没有可比封面人脸的 93 位女优（ADR-0056 的只读估计，88 位盘上没有单人作品封面）：补头像后继的人脸比对缺参照，仍装不上。可选的参照是片商或事务所资料页人像、单人作品的九宫格抽帧；也可以不要封面参照：同名几张图库图彼此过线（同一人在几家片商的图），就足以认定。
43. 另行授权后先备份 ledger，删掉账本里的 202 条附属文件资产行（扫描已不再登记这类文件），只删行、不动盘上文件（不能走 `purge_assets`，它先删媒体文件）：NFO 11、海报与背景 31、`extrafanart` 剧照 93、截图缩略图 67，清单是 `build/sidecar-audit/sidecar-assets.csv`；`ABP-325.mp4_thumbs.jpg` 带 1 条标签与 1 条关系，一并删。`森萝财团 写真 月下-09E` 的 `cover.jpg` 挂着创作者关系，是写真集封面，保留。
44. 另行授权后先备份 ledger，新建女优 `宮澤寧音`（minnano-av `actress172369`，别名 `川北すずね`），把 `B:\云下载\川北すずね\` 下 50 条视频资产挂上：用户 2026-09-24 确认全是她，证据另有 Fantia 公式ファンクラブ人脸比对。`FC2-PPV-4493248`、`FC2-PPV-4573362` 两部合集待定。清单是 `build/avatar-audit/kawakita-link-candidates.csv`。
