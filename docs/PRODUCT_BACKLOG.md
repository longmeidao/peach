...
37. 另行授权后先备份 ledger，把 `asset.snapshot_path` 里 9026 行 `R:\Resources\Intake\snapshots\…` 前缀改写成当前根 `R:\peach-data\generated\snapshots\…`（另 3146 行已是新前缀），前后按行数与文件存在性核对；迁移落地后再删 `legacy_snapshot_roots` 与 `LEGACY_SNAPSHOT_DECLARATIONS` 及五个模块里的运行期重映射。
38. 另行授权后跑 `scripts/backfill_rule34_tag_types.py --apply --backup <落点>`：2026-09-08 只读核查 rule34xxx 的 2396 条 `follow_item` 里 566 条没有 `tag_types`，卡片上因此不显示标签；先 dry-run 看限流情况再 apply。
