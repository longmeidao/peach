...
3. **在线追更**：kemono/coomer/pawchive、rule34video、rule34.xxx、f95zone 五类连接器、WIP/alt/跨站重复判定、`follow_source`/`follow_item`、`peach follow` 与 `/follow` 页面已上线；仍缺定时轮询（APScheduler）、媒体下载落地，以及 simpcity 的可用入口（DDoS-Guard，未取得）。
...
合计：**11 项开放需求**，其中 5 项已有骨架，6 项尚未实现。持久播放列表（2026-08-25 上线，可保存 Mix、命名、排序、续播和编辑）、头像人脸取景（2026-08-22 上线，见 STATUS）和 115 大小写路径失败（2026-08-17/18 收尾，2 条失败已定因）已完成，不再计入。本轮设置、5 分钟刷新、搜索历史、播放器统计、动态环境光和 Shorts 风格沉浸模式不再计入开放需求。
