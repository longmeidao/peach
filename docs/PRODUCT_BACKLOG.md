...
23. 判定 18 组「前缀未证实」的 r18dev 归属（`scripts/audit_scraped_identity.py` 出 `scraped-identity-review.csv`，2026-09-25 共 88 行）：`476MLA-234` 查到 `MLA-234`、`428SUKE-080` 查到 `h_1711suke00080` 这类，去掉三位数字前缀才对得上。`390JAC-040` 与 `JAC-040` 是两个商品，数字前缀的差异不能默认放行；账本里的标题、日期取自同一份快照，不能互证。要拿 MGStage 详情页或 avbase 的跨店条目逐组核对，核实的写法关系登记进 `catalog_rules._RELEASE_PREFIX_ALIASES`，不是同一部的按 `peach-ledger-write` 另行授权撤掉归属。
