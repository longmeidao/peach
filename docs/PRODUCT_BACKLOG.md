...
合计：**38 项开放需求**，其中 7 项已有骨架，31 项尚未实现。已完成的需求不在这里留痕，去 Git 历史查。
...
42. 另行授权后跑 `scripts/poster_crop_boxes.py --apply`：本机 1014 张 JAV 横版封面 dry-run 折痕 577、右半居中 106、不裁 331、读图失败 0，只写 `<番号>.poster.json` 边车，封面原图一个字节都不动；折痕命中的相对位置中位数 0.5253。先抽看 `--limit` 一小批的 `poster_box` 在卡片上的效果再放全量。
43. 另行授权后修复 `local` 来源的路径大小写重复：2026-09-12 只读盘点，`R:\Media\…` 旧行 2,547 条（`last_seen` 全部停在 2026-08-13，挂着 2,547 条标签、2,547 条实体关系、2,542 张快照）与 `R:\media\…` 新行 2,553 条（2026-09-09 声明根改小写后由扫描登记，标签、实体、快照全为 0）按 `lower(path)` 一一成对 2,547 对。SQLite 的 `=` 区分大小写，扫描按声明根拼路径，所以旧行再也扫不到、新行永远是白板。修法是先备份，把新行里没有任何引用的那 2,547 条删掉，再把旧行 `path` 改成账本声明根的写法；剩余 6 条新行是 08-13 之后新增的文件，保留。动手前核对 `asset_preference`、`playlist_item`、`watch_queue`、`asset_subtitle` 有没有指向新行。
