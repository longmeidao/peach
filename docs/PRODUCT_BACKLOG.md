...
42. 逐对判定 `115` 来源的 11 对路径大小写重复：2026-09-12 只读盘点，成因与 `local` 那次不同，是 `.MP4` 与 `.mp4`、`MIDE-950-C` 与 `mide-950-C` 这类扩展名与目录名的大小写，且多数两侧都挂着标签与快照（例如 `86263` 有 5 条标签、`28608` 有 6 条）。不能像 `local` 那样机械地保旧删新，要一对一看哪侧的标签与快照更全，合并后再删另一侧。
43. 悬停时间轴预览：现在悬停扫的是九宫格那九格（`/poster?id=&c=`），一部两小时的片子，格与格之间隔着十几分钟，拖动条上对不上位置。在线站的做法是每 10 到 30 秒一张、拼成雪碧图按时间索引。落到 Peach 要先回答四件事：图存哪（`generated/` 下按 asset 分目录，还是一张雪碧图加一份索引）、谁来生成（接进 `process_library` 的一段，还是像九宫格那样独立脚本加任务）、多大代价（两小时的片子 10 秒一张是 720 张，按 480 宽估 20–100 MB，全库乘以三万部要先算总账和清退规则）、网盘上的片子要不要做（每张都要拉一次 Range，流量预算见 `traffic-policy`）。`PreviewService.poster()` 的裁格子逻辑与 `follow_covers` 的抽帧命令可以复用，不要另起一份裸 ffmpeg 调用。
