...
侧栏底部固定设置和明暗开关，顶部独立按钮控制展开；媒体库图标负责打开选择面板，首页入口使用 Peach logo。路径可命名分组并选择图标，网盘默认图标由本机提供；库选择限定作品列表与筛选项。统计、口味和维护任务保持全局。关注提供作者分组与可多选的来源列表，最近观看标题可跳转视频。详情优先响应 Esc，手机排序栏可横向滚动。预览不保存真实配置。
