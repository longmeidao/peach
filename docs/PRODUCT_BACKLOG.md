...
27. 合并 `飯岡かなこ`（#8559，1 部）到 `森泽佳奈`（#7901，2 部）。证据是 r18 把前者登记为后者的别名，复核件 `peach-data/review/duplicate-identity-20260904.csv`。走 `merge_duplicate_identities.py --apply --backup <落点>`，先取授权。
28. `Ako Shiraishi`（#8435）的规范名写的是另一个人：它的每一个别名（`Suzu Hirasawa`、`ひらさわすず`、`平沢すず`）和 r18 引用都指向 平沢すず，而 `Ako Shiraishi` 是 `白石亚子`（#8204）的别名。这一条要改规范名，不能合并——合并会把两位女优搅在一起。改真相字段，先取授权。
29. AKA-024 的系列同时挂在 `クロロホルムレ●プ`（#8416）与 `Night Safari`（#8636）两条实体上，且前者带一条 `r18dev:series-localization` 写的 `Night Safari` 别名。AKA-022 才是 クロロホルムレ●プ；先判本地化那一步为什么把两个系列对到一起。
30. 96 位演员的规范名是假名、且一条中文／全汉字别名都没有（如 `飯岡かなこ` 的公认译名 `饭冈佳奈子`），按中文名搜索一律落空。厂牌那侧已有 `localize_studio_names.py`，演员这侧没有对应入口。
