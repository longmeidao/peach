...
4. 缺时长资源补 probe 后再生成接触表。
5. PikPak 计费抽样与下载边缘质量核验。
6. Stash CommunityScrapers 的协议复用与元数据导入。
7. 剩余 status / suggest / ledger 逻辑移入应用边界后删除旧 CLI 表面。

合计：**12 项开放需求**，其中 5 项已有骨架，7 项尚未实现。头像人脸取景（2026-08-22 上线，见 STATUS）和 115 大小写路径失败（2026-08-17/18 收尾，2 条失败已定因）已完成，不再计入。本轮设置、5 分钟刷新、搜索历史、播放器统计、动态环境光和 Shorts 风格沉浸模式不再计入开放需求。
