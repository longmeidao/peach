...
25. 浮层筛选收成一个组件。首页浮层是 `#tagbar` 与 `#count` 包进 `.board-filter-frame` 的一个盒子；资料页是 `.entitytagbar` 与 `.entitycollectionhead` 两个兄弟，DOM 和样式各一套，整块玻璃靠下半的 `::before` 往上铺出来。收成同一结构、四个槽（视图胶囊、标签、读数、排序与开关），首页和资料页的视频、照片视图共用；资料页的筛选键加上 `state`（`/api/items` 已支持），由此得到「全部／没看过／稍后看／已标记」。
    重画只换网格和读数，浮层节点常驻：资料页的下半现在每次整块换 innerHTML，筛选、排序、换一批都会重建它。一并收掉已知的重复：标签胶囊有首页 `tagPillHtml` 和资料页各一份模板，关注页另有三份 `.pill`；`.sorts` 有首页、资料页、照片三份拼法；`.entitycollectionhead` 有作品、名册、照片、骨架四处模板；复核页那两条仍各做一块玻璃。动手前再按同一判据扫一遍其余页面，落地后删掉 `.entitytagbar`／`.entitycollectionhead` 的专用样式。

合计：**32 项开放需求**，其中 7 项已有骨架，25 项尚未实现。已完成的需求不在这里留痕，去 Git 历史查。
