...
28. macOS 标识 `io.github.longmeidao.peach.*` 在 Mac 上生效：代码已在 master（`src/peach/appid.py` 是唯一来源，`install_macos_agent.py` 与 `setup_macos_port80.sh` 会自己清掉遗留标签），命令与四项核对见 `docs/OPERATIONS.md`「桌面入口与发布」。放进第 31 条的维护窗口一起做；两台机器都跑过之后删掉 `peach.appid` 里的遗留标签表和用到它的分支。这是换生产入口，执行前要在同一轮拿到用户确认。
...
32. 给事务所补一个 `/agencies` 索引页。现在进事务所页的入口只有女优页上的那个名字，55 家里没有关系的那几十家等于只能靠猜地址。`q_index` 的 `performers` 分支按作品数排、带头像，事务所要的是按成员数排；`openIndex` 里 `people`、`entityKind`、加载文案三处是按 kind 写死的，加一种就要各动一处。
