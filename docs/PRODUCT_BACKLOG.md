...
43. 删掉账本里的 202 条附属文件资产行（扫描已不再登记这类文件），用户 2026-09-24 已授权，auto 模式拦下删除，由用户在终端执行 `build/sidecar-audit/sidecar_delete.py --apply --backup <路径>`：只删行、不动盘上文件（不能走 `purge_assets`，它先删媒体文件），清单是同目录的 `sidecar-assets.csv`，NFO 11、海报与背景 31、`extrafanart` 剧照 93、截图缩略图 67；`森萝财团 写真 月下-09E` 的 `cover.jpg` 是写真集封面，保留。
