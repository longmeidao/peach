...
26. 多个媒体文件夹：首次运行页与配置页都只收一个「媒体文件夹」，写进设置文件 `[media.locations]` 的 `local` 项；外置硬盘和第二块盘上的资料只能靠后来手改 `config.toml`。要做的是把表单改成可加减的目录列表（Geist 的做法是列表加一枚「添加」次级按钮，不放在输入框同一行），`onboarding.apply` 与 `peach init` 同步收多值，扫描与 `asset.location` 按声明根逐个落账。上一条来源通用化（第 11 条）先做，否则每个新目录还得起一个硬编码的来源名。
