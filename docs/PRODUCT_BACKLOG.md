...
35. **Feed 番号发现源**：订阅 RSS，只从条目里解析番号，去重后为每个新番号派一条刮削任务，并在账本里入库为「未入库」空壳，人物页与首页据此提示「这位有新片，你还没有」。已读与忽略分开存：读过不等于不想要，忽略过的不该因为再次出现而复活。前置是先取证到稳定的 RSS 来源——JavDB 没有公开 RSS，能不能拿到可复现的订阅地址还没有结论，取不到就写「未取得」，不拿站点搜索页顶替。

合计：**42 项开放需求**，其中 7 项已有骨架，35 项尚未实现。已完成的需求不在这里留痕，去 Git 历史查。
