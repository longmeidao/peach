...
YouTube Shorts 沉浸页使用用户截图记录 `docs/reference-snapshots/youtube-shorts-immersive-user-screenshot.md`：竖屏复用中央 9:16 舞台和外置动作列，横屏使用在整个视口视觉居中的 16:9 舞台并把动作列收进视频右侧；Peach 保留自己的反馈语义，不复制订阅或社交动作。沉浸视频每次加载都要带独立 `session`，切片、关闭、失败和页面离开时取消旧会话；只清浏览器的 `src` 不足以停止 CloudDrive 预读或 FFmpeg。普通播放器控制栏另使用用户截图、实际 YouTube `e937390a` 播放器 DOM／CSS／JS 和官方帮助记录 `docs/reference-snapshots/youtube-player-controls-user-screenshot.md`：控制栏为透明 59 px 两排结构；左侧播放是 40 px 圆钮，音量从 40 px 展开到 111 px，提示层允许越过胶囊，时间使用 40 px 高、28 px 圆角胶囊并可切换已播／剩余时间；右侧画中画／设置／影院／全屏不是四个描边圆钮，而是 48×40 px 单钮共用一个 40 px 高、28 px 圆角深色胶囊。已播段、12 px 圆点和卡片观看进度统一使用 Peach 蓝。中央只保留一个 68 px 深色圆形播放／暂停键；键盘仍按设置前后跳转，控制栏和中央层都不画 10 秒按钮。本地时刻预览复用九宫格接触表，在线视频只显示时刻。设置浮层按实测 274 px 宽、12 px 圆角、48 px 行高、56 px 图标列和 57 px 子菜单标题栏，只列真实能力：氛围模式全局开关、播放速度和实际清晰度；影院模式用按钮或 `T` 切换为大播放器，展开／收回使用不同图标和「影院模式／默认视图」提示，详情移到下方。音量、画中画和全屏改用统一 24 px 本地图标；Peach 不复制没有实际能力的字幕、睡眠定时或自动播放按钮。
