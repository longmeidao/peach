...
- **瀑布流只读缩略图，原图只在灯箱里读**。图片资产没有接触印相可裁，只能读原图；PikPak 一张动辄几 MB，一屏几十张就是几十兆计费流量。`/photo-thumb` 用 Pillow 缩到 640 宽存进 `photo_root`，每张只回源一次，`/photo` 只服务灯箱当前和相邻的大图。瀑布流用 CSS `column-count`：图片行没有 `width`/`height`，等宽多列流式排版正好不需要比例，也就不必等图加载完再算位置。灯箱继续复用 Swiper 14.2.0，但主画布必须直接固定到视口，顶部工具和底部缩略图只作覆盖层；不要再用三行 grid 承接 Swiper 的百分比高度。关注图集箭头、灯箱翻页和沉浸动作列共用 48 px tonal 圆钮，禁止恢复浏览器默认按钮内边距或额外描边。
