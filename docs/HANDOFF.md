...
- **标签添加与筛选（2026-08-15 / 2026-08-27）**：macOS 标签添加弹窗只复用用户截图可证实的搜索、最近使用、全部标签、已选状态和键盘操作。Hanime1 搜索页由 Chrome 实时打开 `https://hanime1.me/search`，标题为「搜尋 - H動漫/裏番/線上看 - Hanime1.me」；页面与标签弹窗在用户端正常，浏览器控制通道展开后返回 DOM/截图连续超时只记为**控制结果未取得**，不得写成网站或渲染无响应。筛选交互以用户截图 `codex-clipboard-adaadb20-6a78-4791-8438-664736a45cbd.png`（337,723 字节，SHA-256 `0ff2a1267d330e627b21d905554faaaca036813531e7182e6b2574196e696aec`）为证：多标签选择、可切换「广泛匹配」（任一 / 全部）并确认显示结果。Peach 复用这三条行为；人物关系和角色设定不属于当前标签页，明确不复制。后续 tagging 只把该词表当候选来源，仍须经过来源、规范化和人工复核边界。
