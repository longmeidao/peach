...
- 「归一化」与「判形态」是两件事，混用会让判据自己打自己。`normalise_code` 会补上分隔符，`RAIKUN325`（myfans 账号名，241 个文件）被改写成 `RAIKUN-325` 后就通过了番号形态检查；下划线同理，`BANBI_555` 会变成 `BANBI-555`。分隔符本身就是区分番号与账号名的唯一线索，归一化把它抹掉了。所以形态判定必须看原值，归一化只服务封面查找那类需要稳定键的场景。这个坑在 `web_contract` 与 `fetch_jav_covers` 里各犯过一次，现已共用 `is_jav_code` 一份实现。代价是 `SOAN045`、`DTW024` 这类漏写分隔符的真番号会被判为非 JAV——二者结构完全相同，无法自动区分，宁可漏掉几个也不能把账号作品塞进 JAV 模式。
- 前端「刷新」要区分「重取」与「换一批」。`q_tops` 是 `ORDER BY n DESC` 的确定性查询，失效缓存后重取拿到的还是同一批人，界面上纹丝不动。要真的换人必须引入种子：放大候选池再按种子确定性抽样，同种子可重复（翻页与重绘之间不能抖动），缓存键也要带上种子和口径，否则几套结果会互相顶掉。
- 上一条只写成知识没有强制机制，于是同类写法又长了回来。AST 审计发现 6 处仍是裸 `text=True`，其中 `src/peach/versioning.py` 是生产代码——它读 git 输出，而本仓库的提交信息就是中文的，拿 `282f8d9` 自己的标题即可复现 `stdout=None`。已全部锁定 UTF-8，并加 `tests/test_subprocess_encoding.py` 扫全仓强制。该守卫只认 `subprocess.*` 字面调用，包装器（如 `VersionManager._git` 走注入的 `self._execute`）看不见，那个盲点由同文件里一条显式断言补。
