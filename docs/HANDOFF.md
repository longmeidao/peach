...
- 整页 HTML 缓存与限速走 `peach.page_cache.Site`（按 URL sha1 落盘，`cookies` 用来带过年龄门）。它原先长在 `harvest_directory_links.py` 里，厂牌名回查是第二个用户，已上提到 `src/peach/`。采集脚本的判据改一行就要重跑，缓存在手才能让重跑走离线数据、不再打外站。
