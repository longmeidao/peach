...
- Windows 日常入口是当前用户 Startup 中的唯一 `Peach.lnk`，指向 `C:\Users\longm\Desktop\peach\peach-app\dist\Peach\Peach.exe`。`R:\peach-app` 是迁移前的旧位置，不再用作运行入口。智能体刷新源码运行态时不得用 Computer Use 点托盘，只能走与“重启服务”等价、且保持或重新建立托盘子服务所有权的项目命令；直接强杀／另启 `peach.exe` 不等价，命令缺失时先补安全入口。
