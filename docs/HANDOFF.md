...
**Beeg 资料页（2026-08-29）**：Reislin 页面实测顺序是头像与名称、简介、外部链接、关联艺人、标签、内容。Peach 的女优／厂牌资料页和关注页复用这个阅读顺序，但媒体切换只按 Peach 的真实视频／图片数量出现；合集详情继续使用普通详情的左媒体、右信息，下方另设 Mix／分卷／播放列表／多视频的横向切换带。锁定样式摘要、截图摘要和主动差异见 `beeg-profile-layout.md`。
