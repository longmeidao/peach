...
YouTube Shorts 沉浸页使用用户截图记录 `docs/reference-snapshots/youtube-shorts-immersive-user-screenshot.md`：竖屏复用中央 9:16 舞台和外置动作列，横屏使用在整个视口视觉居中的 16:9 舞台并把动作列收进视频右侧；Peach 保留自己的反馈语义，不复制订阅或社交动作。沉浸视频每次加载都要带独立 `session`，切片、关闭、失败和页面离开时取消旧会话；只清浏览器的 `src` 不足以停止 CloudDrive 预读或 FFmpeg。普通播放器控制栏另使用用户截图与 YouTube 官方帮助记录 `docs/reference-snapshots/youtube-player-controls-user-screenshot.md`：进度独占上排，播放／音量／时间在左下，画中画／设置／影院／全屏在右下使用同高圆形按钮；已播段、圆点和卡片观看进度统一使用 Peach 蓝。键盘仍按设置前后跳转，控制栏不重复画 10 秒按钮；本地时刻预览复用九宫格接触表，在线视频只显示时刻。设置浮层只列真实能力：氛围模式全局开关、播放速度和实际清晰度；影院模式用按钮或 `T` 切换为大播放器，详情移到下方。Peach 不复制没有实际能力的字幕、睡眠定时或自动播放按钮。
