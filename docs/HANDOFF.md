...
- `metadata_fields` 的 `item_key` 是 `<番号>:<字段>`，不带候选身份，所以一条旧 approved 会把之后抓到的新来源值一起盖住：TRE-080 的标题 2026-09-01 对着 r18dev 的空日文标题批过一次，之后 javbus 抓到真标题，`/review` 里一条也看不见，页面上仍是英文标题，同批还有 24 个番号。判据是 `review_decision.note`（JSON，记着批准时选的 `candidate_key`）指向的候选是否还在这一组里，不在就清掉旧判定重新入队，与 `studio_logos` 的「上游内容变了就重开」同源。note 是早期自由文本、读不出指向的一律放过，宁可漏一条也不要把批过的翻出来。
- 目录多余层的判据是**名字冗余**，不是「父目录只有一个子目录」——真实账本里后者有 747 个，多数有意义（`古川结爱合集` 底下就一个 `FC2-PPV-…`）。摘掉广告标记、抹掉大小写与分隔符后子目录名等于父目录名才算多余（`TRE-080\[44x.me]tre-080`），见 `scripts/flatten_release_dirs.py`。广告标记只剥头尾：`[Vixen.com]`、`[StraplessDildo.com]` 是厂牌名不是广告，而裸域名不能允许标签里带连字符，否则 `ABP-762-fuckbe.com.mp4` 整串都符合「标签 + .com」，剥完只剩 `mp4`。账本只记文件不记目录，任何一条计划落地前都要用真实文件系统再核一遍父目录里有没有旁挂的封面、字幕和空目录。
...
- **来源返回的番号必须和查询的番号比对过才算命中**。javbus 一侧是拿番号做关键词搜索并取首个结果，搜不到就返回近似的别人：`SA-104 → AVSA-104`、`CHU-101 → CHUC-101`、`AR-301 → STAR-3016`，2026-09-02 实测 68 次匹配 58 次番号根本不对，整个 `B:\MVP\MIB\`（韩国内容）因此被写上日本厂牌、系列和标题，还靠这些假证据升级成 JAV。判据是 `catalog_rules.same_release_code()`，它容忍片商数字前缀、DMM 的 `h_` 标记、补零和重制尾字母这些良性差异；来源没给 id 的不拦（缺证据不是反证）。
- **同一部作品有两种番号写法，账本存哪种不代表来源索引哪种**。`259LUXU-1642` 的三位数字前缀标的是 DMM 上的发行方，不属于作品身份，来源站各只收其中一种写法。`catalog_rules.code_query_variants()` 在两种写法间回退，评审键始终用账本的规范写法。去前缀总是安全的（只是丢掉本来就有的一段），补前缀要凭空填三位数字，只对 `MAKER_NUMBER_PREFIX` 里登记过的字母段做；新增一行之前先在账本里确认该字母段只属于一个发行方，猜错就是拿别人的番号去查。
