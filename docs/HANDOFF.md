...
- **Javinizer-Go 元数据 Provider（2026-08-22）**：取得官方仓库 `https://github.com/javinizer/javinizer-go`、v1.5.1 release 与 tag commit `f57985c8c98a0ad7283f72854c249b8612300637`；本机 `javinizer-darwin-arm64` SHA-256 `6197d78c561a05a42c9ef0e3896991d6f5e46d863595d25ed13de0139d0f179d` 与发布清单一致，并用 `IPX-535` 验证单来源 JSON。Peach 复用 query-only `scrape --output json --scrapers <source>` 和结构化错误；主动差异是仅发送规范番号、自己逐源留原始证据和形成字段候选，批准后才写 ledger，且不使用其 DB/organizer。
## 智能体用量与任务路由
