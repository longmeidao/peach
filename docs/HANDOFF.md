...
- 自动换一批默认 5 分钟一次，只在首页空闲态执行；详情播放、沉浸模式、搜索输入、多选和其他页面不会被打断。设置面板只保留用户可理解、会改变体验的参数：刷新开关与间隔、每批作品数、默认排序、悬停放大延时、快进/快退秒数、搜索记录条数和相关推荐数量。环境光是播放器默认效果，不作为技术开关暴露。
- 搜索框聚焦后显示本机搜索记录，并在下方显示随机推荐；Enter 提交真实关键词，搜索记录不写 ledger。
- 详情关闭统一为 X；播放统计继续复用 Video.js 和浏览器原生指标，不再引入第二套播放器。动态环境光由同源视频低分辨率取色画布生成，不复制外站图片。
- 新增正式迁移 `0011_better_version_request.sql`：逐条记录“寻找高清 / 无水印 / 完整版”等质量目标，不删除或替换当前资源。
- YouTube Mix 的产品定义是可连续播放、可保存的个性化播放列表。Peach 当前“接着看”只是相似推荐，不能宣称 Mix 已完成。开放需求统一见 `docs/PRODUCT_BACKLOG.md`。
