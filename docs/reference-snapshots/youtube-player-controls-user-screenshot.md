...
| 时间 | 56 px 高、8 px 垂直 padding、透明 | 去掉旧的独立时间胶囊和描边 |
| 右侧控制组 | 高 40 px、左右 padding 4 px、圆角 28 px、背景 `rgba(0,0,0,.3)`、模糊 16 px | 画中画／设置／影院／全屏放进同一共享容器 |
...
| 左下 | 播放、音量、当前时间／总时长共用一条基线 | 播放为 40 px 圆钮；音量胶囊从 40 px 展开；时间透明；前后 10 秒移出控制栏，键盘快进仍保留 |
