...
侧栏采用 AI chat 公开变体的分组标题、展开叶项与尾部计数：标题 14/20，计数 12/16、1×4 内边距、4px 圆角。媒体库入口适配公开 `DashboardUserMenu`：265px 面板、16px 圆角、10px 内边距，桌面右侧 8px、手机下方展开，150ms ease-out 淡入、缩放 .95 与 2px 模糊。收起时点击 Logo 展开侧栏，不保留空的选择行。设置固定在底部，与明暗开关并排；收起时明暗开关只显示目标主题图标。主题动画参考公开 `https://www.boardui.com/r/theme-toggle.json`：200ms 滑块与 820ms 柔边扩散，缓动 `cubic-bezier(.16,1,.3,1)`，减少动态效果时直接切换。

媒体库使用 `[media.libraries]` 为声明路径命名，同名路径归为一库。库选择限定作品列表与筛选项；来源 ID、挂载映射和 ledger 路径保留其业务含义。统计、口味和维护任务仍按整个部署汇总。配置表单读写库名，预览只读，不保存真实配置。

详情使用并列观看进度卡与独立动作按钮；Esc 先退出详情、再收起侧栏。首页与实体页排序保持横向滚动，换批按钮沿用动画 SVG，采用中性 Board 按钮。

- 162 项前端测试通过，包含非法值不保存、关闭恢复、异步读取值恢复、图表数值、库名提交、分组展开记忆与可访问语义；构建与类型检查通过。
