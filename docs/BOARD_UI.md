...
在 Peach 的每个页面持续显示，全部任务完成或跳过后自动消失，不再要求一次额外确认；
...
统计圆环直接显示终值，只有高亮那一段的浓淡走过渡；入场动画随 React 版取消，
理由同排名条与雷达图那一行。
