...
- Windows HTTP 为 `0.0.0.0:80`，HTTPS 为当前 LAN IPv4 的 443，mDNS 名见 `[server].mdns_name`；线上版本 `0.8.3`、`ledger_sync=writer`。
...
- 控件按图 3 恢复灰阶；29999 及另 16 个番号的 23 个视频补齐女优，账本完整性通过。实屏未验。
