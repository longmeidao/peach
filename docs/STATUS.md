...
- r18.dev 打包在一个罗马字字段里的 17 条复合艺名已拆成 37 条独立别名（`split_composite_aliases.py`，备份 `ledger.pre-split-composite-20260904.db`）。只拆「括号前有空格、括号内逗号分隔、两侧都是罗马字」这一种形状；系列的厂牌消歧、标签的作品出处、创作者的接稿状态和去重后缀共用同一个字面形状，一律不动。
