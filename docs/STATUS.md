...
- 生产入口：Windows 当前用户 Startup 中的 `Peach.lnk`，启动 `R:\peach-app\.venv\Scripts\pythonw.exe -m peach.tray`。当前版本 `0.5.6`。
...
- 详情播放器使用本地固定版本 Video.js 8.23.9，提供 ±10 秒、画中画和播放统计。远端 MP4 恢复标准 HTTP Range，不再把浏览器的开放区间伪截成 32 MiB；item 29297 的权威总时长固定为 28,639.916 秒（`7:57:19`），不再随缓存增长。
...
- 当前主分支基线：135 项隔离 `unittest` 通过，inline JavaScript 语法检查通过；版本、托盘、迁移、mDNS、媒体转码、Provider、DiskGuard、语义路由、标准 Range、Video.js、稳定时长、详情播放释放、多选、实体资料、分页性能边界与待删视觉均有测试。
...
- 本批已重启生产托盘。HTTP 与 HTTPS `/healthz` 均返回 `0.5.6`；HTTPS 使用项目 CA 严格校验，`peach.local` 解析为 `192.168.50.162`，80/443 分别由新托盘子进程监听。生产只读 Range 对 item 29297 返回 `206`、精确 `bytes 0-1048575/4590823524`；桌面与 390×844 均固定显示 `7:57:19`，验证前后播放次数、播放秒数、最远进度和 seek 次数不变。
...
7. 增加 Media Engine stream-plan API，由现有 Video.js 内置 VHS 接入真实 HLS/DASH 短分片；不得自己造播放器协议层。CloudDrive 约 100 MiB 固定块预取仍是来源层成本，换播放器外观不能消除，必须以直连来源 adapter 或服务端分片解决。
...
<!-- generated 2026-08-15T12:24Z -->

- 最近自动交接：`claude` / `Stop` / `completed`，2026-08-15T11:54:47+00:00。
...
- 无内容标签视频 7593 条（占视频 30%）。
- `asset_tag` 来源分布：`vision_creator` 27004、`pixiv_tag` 19753、`name` 19329、`stash` 15450、`r18` 2538、`performer` 1887、`follow` 1376、`r18:performer` 1216、`javbus:performer` 50。
