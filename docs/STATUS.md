...
- `/taste` 新增只读的口味总结与行动建议：结论分别列出浏览主线、Peach 印证和达到最小证据门槛的共同信号；可信度同时考虑浏览量、观看项目与明确反馈，当前真实副本因只有 4 次明确反馈而显示「可信度中等」。找内容入口只推荐首页默认范围确有命中的规范 Tag，当前真实副本给出 `3D动画` 176 部、`口交` 1124 部、`游戏同人` 133 部，并直接复用首页筛选；常访创作者进入关注管理，58 项待补 Tag／89 项待补身份进入人工复核。建议不调用外部 AI、不写 ledger，也不把具体项目的「不合口味」外推为 Tag 降权。本轮更新 8900 预览，生产 80／443 未重启。
