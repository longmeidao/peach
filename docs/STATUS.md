...
- 名字里的括号分两拨处理，都走 `split_composite_aliases.py`。自动那拨只认「括号前有空格、括号内逗号分隔、两侧都是罗马字」，把 r18.dev 打包成一个字段的 17 条复合艺名拆成 37 条别名（备份 `ledger.pre-split-composite-20260904.db`）。`--from-review` 那拨按人工判定又清掉 9 条不承载名字的尾巴：展会摊位号、接稿状态、更新日期、投稿分区，以及重复的厂牌名（备份 `ledger.pre-name-cleanup-20260904.db`，旧写法留作别名、来源 `user:name-cleanup`，32 个资产的扁平投影跟着改）。剩下的 28 条创作者括号里是读音、10 条系列是厂牌消歧、575 条标签是角色出处，都不拆。
