...
- Windows HTTP 为 `0.0.0.0:80`，HTTPS 为当前 LAN IPv4 的 443，mDNS 名为 `peach-win.local`。线上服务版本最后核验为 `0.7.5`、`ledger_sync=writer`。
...
- 管理页 `/taste` 以浏览器记录为主、Peach 内部行为为辅，分区展示各自排序；创作者与女优显示实体头像，常访问网站显示 favicon。宽屏改为按信息密度分配 4／6／8／12 栏且面板按自身高度收口，不再固定等宽双列；返回品味页会清除临时下钻筛选。明确“不合口味”只归于具体项目与理由，不自动给 Tag 降权；模糊时长旧 Tag 从品味结果和前端筛选状态排除。正式 Windows 全量 1142 项通过、13 项按平台跳过；1440×900 实测网站 favicon、创作者／女优头像与非等宽面板正常，390×844 为单列且无横向溢出，Tag 下钻后返回品味再进首页不残留筛选。原始 URL／标题仍不进入页面或 ledger，本轮未改真实 ledger 中的旧实体行，也未重启 80／443 生产服务。
