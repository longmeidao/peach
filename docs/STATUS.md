...
- 艺人、厂牌、创作者、系列资料页的作品区统一复用首页排序：换一批、推荐顺序、每日轮换、随机、评分、高潮计数、观看次数、时长、体积、最近入库、最近看的；照片瀑布流和管理队列不添加无适用语义的排序。资料页 API 继续保留完整别名供搜索，但页面不再展示本地化规范名的纯拉丁转写；番号卡片优先显示规范女优实体，不再被旧 `asset.creator` 罗马字投影抢占。实体解析改为精确规范名优先、仅接受唯一别名，修复 `飯岡かなこ` 被 `森泽佳奈` 同名别名抢走的问题。390×844 实测页面宽 380、无横向溢出，排序行 348/710 px 可横滚；艺人、厂牌、创作者、系列均有 10 个排序按钮，JAV 厂牌另保留 3 个版式按钮。正式 Windows 全量测试 1142 项通过、13 项按平台跳过。
