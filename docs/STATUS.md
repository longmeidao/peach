...
- `/taste` 排名改为按语义区分密度：Tag 保留使用全局 Tag 圆角的可点击胶囊并在宽屏并列，普通排行使用共享数字／内容／操作列和轻分隔线，头像与 favicon 继续占独立身份列；右侧指示统一贴边。数据源在宽屏按最小 270px 自动分栏，窄屏回落单列；顶部更新时间、时间范围、读取与导入操作保持左对齐。Vercel 报告设计快照本轮核对未变化，仅复用共同网格、基线和避免无意义重复卡片的原则。1137px 实测浏览器 Tag 为 3 列、Peach Tag 为 2 列、7 个数据源为 3 列且每列约 316px；390×844 下两者均为 314px 单列，页面宽 380／390 无横向溢出。catalog 387 项通过；控制台专用错误列表因当前浏览器接口无此入口而未取得。本轮只更新 8900 预览，生产 80／443 未重启。
