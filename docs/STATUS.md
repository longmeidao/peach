...
- `/taste` 排名内部不再为每项嵌套圆角卡：普通项使用透明分隔行，悬浮／键盘聚焦才出现左侧强调；排名数字使用固定 32px 居中列，无头像与有头像条目分别使用三列／四列结构，右侧指示统一贴边。顶部更新时间、时间范围、读取与导入操作改为左对齐。1137px 实测排名左／右边距均为 4px，390×844 为 314px 单列且页面 380／380 无横向溢出；catalog 387 项通过。本轮只更新 8900 预览，生产 80／443 未重启。
