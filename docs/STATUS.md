...
- Production Peach: FastAPI `0.2.0` from `R:\peach-app` on `0.0.0.0:80`, PID observed as 51308; health mode is `fastapi`.
...
- 21 isolated contract tests passed under Python 3.14 after removing three legacy-server-only tests with the deleted server.
