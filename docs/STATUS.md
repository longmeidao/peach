...
- 头像与紧凑标识按源像素补底，小图标保持原样；小鸠麦使用已核对来源的 600×800 人像。测试 1304 项、前端 74 项通过；桌面及 390×844 预览通过，正式浏览器取证超时。
...
- Windows HTTP 为 `0.0.0.0:80`，HTTPS 为当前 LAN IPv4 的 443，mDNS 名见 `[server].mdns_name`；线上版本 `0.12.5`、`ledger_sync=writer`，项目 CA 严格校验的健康与就绪检查通过。
- 2026-09-06 从 `86600a3f` 载入源码运行态，正常重启托盘并取得 HTTP/HTTPS 子服务所有权，未替换 EXE。换二进制走 `scripts/deploy_windows_tray.py`，且要单独授权。
