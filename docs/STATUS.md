...
- Windows HTTP 为 `0.0.0.0:80`，HTTPS 为当前 LAN IPv4 的 443，mDNS 名见 `[server].mdns_name`；线上版本 `0.11.2`、`ledger_sync=writer`，项目 CA 严格校验的健康与就绪检查通过。
- 托盘 EXE 与检出同步：2026-09-06 07:23 由 `scripts/deploy_windows_tray.py` 从 `25904b67` 打出并原地换上，严格 HTTPS 的 `/healthz` 报 0.11.2；同日 04:53 托盘自重建也换成功过一次。换二进制走这条入口，且要单独授权。
