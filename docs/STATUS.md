...
- 导入或跳过后隐藏指南，折叠保留；浅色导航选中填灰；SFW 模式附说明。测试及桌面、手机预览通过。

- 已恢复 41 张被封面覆盖的人像及焦点。封面禁止安装为头像，尺寸不符不套焦点；半脸场景已复现并修复。全量 3299、追加 2391、前端 77 项通过。
...
- Windows HTTP 为 `0.0.0.0:80`，HTTPS 为当前 LAN IPv4 的 443，mDNS 名见 `[server].mdns_name`；线上版本 `0.15.2`、`ledger_sync=writer`，项目 CA 严格校验的健康与就绪检查通过。
- 2026-09-06 从 `6e99e863` 载入源码，托盘已重启并拥有 HTTP/HTTPS 子服务，未替换 EXE。
...
- 可选访问密码已实现，待集成重启：首启可留空，配置页可设置、修改或关闭；登录页选择设备会话期限。现有安装保留系统口令保护，内部 API 令牌独立。配置 Fieldset 使用 20px 正文间距、8px 标题说明间距，字段错误就地标红。
