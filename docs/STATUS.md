...
- Production Peach: Windows tray launcher PID 17984 (runtime child 19388) owns FastAPI `0.2.1` from `R:\peach-app` on HTTP `0.0.0.0:80` (listener PID 44848) and HTTPS `192.168.50.162:443` (listener PID 48000); both health checks return `200` in `fastapi` mode. The verified current-user Startup shortcut launches `R:\peach-app\.venv\Scripts\pythonw.exe -m peach.tray` at login.
...
- The Windows tray and login autostart are deployed. Single-click is pystray's native default action and opens `https://peach.local/`; the native right-click menu exposes status, restart, logs, a version/update submenu and exit. Version is single-sourced at `0.2.1`; update checks run off the Win32 message thread and report through non-modal tray notifications. A fresh process verified Per-Monitor V2 awareness at system DPI 192 (200%). Startup shortcut, singleton lock, HTTP/HTTPS health, `peach.local` resolution and a real CloudDrive Range request were verified. Windows tray screenshot automation remains unavailable, so native-menu sharpness still needs a user visual check.
...
The current UI/API candidate passed JavaScript parse, 89 isolated tests, desktop 1280×720 and mobile 390×844. Production HTTP, HTTPS and mDNS were restarted onto this code. HTTPS chain verification passed with the corrected local CA; browsing tests remained on the isolated candidate and did not write real playback, preference or feedback data.
