...
- 新 EXE 已构建到 `peach-data/state/source-sync-build/manual-5926ff4/Peach.exe`，打包迁移状态无待处理项，检查前后 ledger SHA-256 不变。项目 CA 严格 HTTPS `/healthz` 与线上静态页面检查通过；因本轮未获生产二进制替换和服务重启授权，当前托盘仍运行旧 EXE，新的后端预览逻辑尚未切换。

## 下一批工作

1. 用户明确授权后，用已验证的新 EXE 可回滚替换 Windows 生产入口并重启托盘；再核对打包迁移、严格 HTTPS、关注非视频预览和手机视觉。
