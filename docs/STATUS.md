...
- Windows HTTP 为 `0.0.0.0:80`，HTTPS 为当前 LAN IPv4 的 443，mDNS 名见 `[server].mdns_name`；线上版本 `0.14.0`、`ledger_sync=writer`，项目 CA 严格校验的健康与就绪检查通过。
- 2026-09-06 从 `45b3ef53` 载入源码，托盘已重启并拥有 HTTP/HTTPS 子服务，未替换 EXE。
