...
- 沉浸模式按用户提供的 YouTube Shorts 桌面截图重排：竖屏使用居中的 9:16 播放舞台，横屏使用靠右的 16:9 大舞台；48 px 圆形操作按钮与文字分离，作者头像、身份和标题留在页面左下；390×844 手机继续使用全视口播放器。保留 Peach 的三项本地反馈、混合横竖屏队列和完整画面优先，不复制评论、分享、Remix 或订阅。
- 正式 Windows `catalog` 范围 368 项通过。临时静态页面验收：1440×900 竖屏舞台 506×900 且居中，动作列在舞台外侧；横屏舞台 922×518、右侧留白 24 px，动作列收进视频右侧；390×844 手机舞台为完整视口。三个视口均无横向溢出，真实页面资源无 JavaScript 语法错误；生产尚未重启。
...
2. 继续按复用审计依次替换浏览器历史、PID 锁和 Rule34Video 媒体页；每项固定版本/revision、首个消费者和隔离测试同批落地。
