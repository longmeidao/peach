...
- Production Peach: Windows tray PID 47092 owns FastAPI `0.2.0` from `R:\peach-app` on HTTP `0.0.0.0:80` (listener PID 43300) and HTTPS `192.168.50.162:443` (listener PID 45312); both health checks return `200` in `fastapi` mode. The verified current-user Startup shortcut launches `R:\peach-app\.venv\Scripts\pythonw.exe -m peach.tray` at login.
...
- The Windows tray and login autostart are deployed. Single-click is pystray's native default action and opens `https://peach.local/`; the native right-click menu exposes status, restart, logs, update status and exit. Startup shortcut, singleton lock, HTTP/HTTPS health, `peach.local` resolution and a real CloudDrive Range request were verified. Windows tray screenshot automation was unavailable and the in-app browser timed out on local navigation, so this deployment did not add a new visual desktop/mobile browser result.
