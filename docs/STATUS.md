...
- 「垃圾文件」有独立 `/junk-files` 路由并按媒体类型分档；「不是垃圾」按 asset 存成可撤销的复核决定，不扩散成来源或域名白名单。
...
- JAV 官方封面的重探队列、只按像素面积更大才原子替换和失败保留原图的判据全在项目技能里。
...
- 关注作者显示名与头像优先取经过固定主机校验的官方页面，归档站只作回退；来源 ref 的大小写与 `Collection(s)` 一类后缀不影响归组，五个来源可归到同一作者组。
...
21. 厂牌 icon／logo 位已按 `studio-icons.csv` 装入（FC2 换源、BangBus／BangBros18 字标补白进两位、Hon Naka 补图）；MonstersOfCock 频道标识未取得，回落母品牌字标。
