...
- `58a4c4f`、`8da8c81` 已合入 `master`：Rule34.xxx 的 HTTP 200 空正文按零命中处理，管理页把查找结果合回添加区并统一标题字号；来源名称可回到原页面，同一作者只保留一个标题、宽屏下来源行分两栏。正式 Windows 测试 1051 项通过、13 项跳过；生产桌面与 390×844 手机视口均无横向溢出，严格 HTTPS 为 writer，重启前后 ledger SHA-256 不变。

## 下一批工作

1. 用可控制 390×844 视口的真实客户端补做关注页手机视觉；同时分类剩余 44 个无预览变体，区分确无图片与来源解析遗漏。
2. 在 `/review` 人工处理本批 JAV 日文系列名、官方标签及现有创作者标签、FC2、Javinizer、Logo、头像和媒体失败候选；未经批准不写真相字段。
3. 将 Windows writer 的 `0020` 副本同步到共享传输点，再让 Mac reader 拉取；同步前后核对迁移版本、计数、完整性与 writer 身份。
4. 在 Mac Finder 以 `smb://peach-win.local/peach-sync` 连接一次并保存钥匙串记录，再重启菜单栏进程，核对自动挂载、reader 锁定、HTTPS、mDNS 和真实 LAN 客户端。
5. 为追更接入 APScheduler；在实现下载器前先确定媒体凭据、流量与磁盘预算。
6. Windows writer 运行 PikPak 夜跑前重算 probe/抽帧队列，并按 `peach-batch-jobs` 设置流量与系统盘闸门。
7. 继续拆分 `web_contract.py` 的 catalog、stats、activity、review、trash 领域，保持路由和现有契约测试不变。
8. 补做 HLS 首帧、seek、自适应码率及桌面/手机视觉验收。
9. 外置盘挂载后先只读盘点 `R:\Media\<名字>\P\...` 图片规模；扫描写真 ledger，需另行授权。
