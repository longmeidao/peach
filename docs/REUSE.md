...
- 外链圆标与厂牌标识取站点自己声明的那份资产，宽扁字标不参加小圆标的竞选；`/logo` 的 `variant` 分 `icon`、`logo` 与按文件像素量挑最清晰那份的 `large`，索引页大图版式的厂牌格与资料页大位取 `large`，紧凑版式的圆框仍取 `icon`；标识不足框八成时按原尺寸居中摆，四周用同图放大模糊补底。
