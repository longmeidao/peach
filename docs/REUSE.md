...
- 候选是抓取那一刻的产物，抓取口径改了它们不会自己跟上：`peach.stale_candidates` 按「未收录 genre 里有整串 ASCII 的词」认出还停在英文那一层的候选（日文原词永远不是 ASCII），`scripts/drop_stale_genre_candidates.py --apply` 把它们从候选文件摘掉并留备份，下一趟「只采集」按 `_missing_fields` 重抓。补表救不了这一类：`Sex Toy`、`Ahegao` 这些词根本不在任何一张表里。
