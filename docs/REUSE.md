...
- 复核页面上下文复用原生 CSS sticky 与主导航 `--topH`，分类及筛选栏合为同一吸附区。卡片以 flex 分开标题、既有 Scroller、当前信息和操作；只有资料滚动，当前信息过长时可聚焦滚动阅读。多选计数共用 `selectiondockcount`，保留首页的文字口径；不新增依赖，不改变候选与提交协议。
