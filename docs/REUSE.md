...
- 账本没有图片宽高，两处图片墙（资料页照片、关注在线图片）一律等宽网格配固定比例格子，不用多列流式排版：缩略图懒加载到达之前每格高度是 0，分栏按零高摊格子，会把一屏格子整批塞进最后一列。格子按比例占位之后，等图那一刻浏览器有地方画 `alt`，一屏就是满屏文件名；字色调透明隐去，`alt` 本身留给读屏。
...
| 竖海报取景 | `opencv-python-headless==5.0.0.93`（Apache-2.0）的 `cv2.Sobel` 求列向梯度；NeoAVDC `c7a430c64013c97a0213cd8a57e2ff5696793a86`（MIT）的封套几何与 sakuramediabe `7e40ef87c7518c1dc2b6c8c299170ad7395fec6d`（GPL-3.0，只作算法边界参考，不引入代码与运行时）的切点接受条件 | `peach.jav_poster_crop` 给出 2:3 竖海报的取景框，`scripts/poster_crop_boxes.py` 批量落边车。**产物是坐标不是图片**：封面原样保存（`jav_cover_fetch` 的约定），框写进 `<番号>.poster.json`，和人脸取景的 `.face.json` 同目录、同命名风格。**两份外部先验都采信，各挡一类失败**：NeoAVDC 量 DMM/JavBus 封套得出折痕在全宽约 52.5%，sakuramediabe 的接受条件是「左右两峰关于中线对称」或「右峰离中线约 20 像素」——那 20 像素是在 800×539 的低清封套上量的，正好是 2.5% 宽，所以 Peach 把它写成宽度比例，否则 2184 像素宽的高清图上同一道折痕离中线 55 像素，会被全判成没有折痕。再加一道「峰值要到全图最强列梯度的 35%」挡住平缓横图。**本机 1014 张封面实测**：命中折痕 577 张，折痕相对位置中位数 0.5253（最小 0.500、最大 0.5875），与 NeoAVDC 的 52.5% 对上；回退右半居中 106 张；不裁 331 张（韩国 MIB 90、FC2 130、16:9 官方剧照 111）。欧美片的编号和厂牌番号同形，番号那一关拦不住，靠宽高比这一关拦下。 |
