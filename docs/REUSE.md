...
| 播放器设置与影院布局 | Video.js 8.24.0 的 `playbackRate`、既有 QualityLevel 与原生控制栏插槽；YouTube `e937390a` 实际 DOM／CSS／JS 只作可测量行为证据 | Peach 在现有 DOM 上组合氛围模式、播放速度、真实清晰度和影院模式，并复用 59 px 两排控制栏、40→111 px 横向音量、4→6 px 进度动画、右侧共享胶囊、整行悬停的 274 px 设置菜单和视口级全屏；图形重画为本地 SVG，不复制代码、不迁移到 Video.js 10 Menu，也不引入重复现有质量选择的插件 |
