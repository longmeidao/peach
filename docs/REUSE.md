...
复核多选共享顶栏模式、Select 与 `/api/review/decision`，支持跨组通过／拒绝与无歧义的共同来源选择；按作品／字段显式选择候选，再逐项采用；成功移出、失败保留选择，页面离开后停止后续提交。分组只区分单一候选与需选择来源，不把来源数量等同于内容冲突。滚动正文复用 Scroller，完成结果复用成功 Note。
