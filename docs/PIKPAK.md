...
待 probe / 合计」。写死在文档里的队列基线只在生成的那一刻成立，后面读到的人会把它当成现值，
从 Mac 副本取的那种基线尤其如此。口径上要分清 `duration=0`（还没探测）和 `duration<0`（探测失败，只有 `--redo all` 会重试），
...
实际 PikPak chain、`C:` 剩余空间、备份路径、退出码，以及是否触发流量或磁盘闸门。少了这几项
就无法判断下一晚该续跑还是该先查问题。
