...
和文件系统完整路径；这四样一旦入镜就是把本机坐标发到公网，撤不回来。介绍视频这类少量演示
媒体放 `docs/assets/`，单个文件压到 8 MB 以内（`tests/test_repo_hygiene.py`）。
...
- **演示素材**：README 里任何截图或视频都按上面「演示数据集」一节的边界取图，换素材时重新核对
  那几条边界；`docs/assets/peach-overview.mp4` 是留档的 58 秒介绍视频，README 正文不再挂它。<!-- copy-lint-disable-line -->
