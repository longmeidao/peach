...
| 深色阴影 | `../styles.css` 在 `@theme` 里把 `--shadow-*` 接到 `--elevation-*`，浅色在 `:root` 上逐字取上游原值，深色在 `.dark` 上另定一档 | Tailwind 把 `@theme` 的阴影字面值抄进工具类，上游 `.dark` 里重定的 `--shadow-*` 够不着 `shadow-card`；上游那组深色 alpha 落在 #111 的页面底上也看不出来 |
