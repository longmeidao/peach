...
const followToolbar = () => `<div class="fsechead" data-collapse-toolbar><h3>关注列表</h3><span class="fmeta">${line('100px')}</span><span class="fmanagesort" data-collapse-field>${icon('sort')}${selectFieldHtml([['checked','检查时间']],'checked',{label:'关注列表排序',attr:'disabled'})}</span><button class="fbtn fmanagedir" disabled>${icon('arrow-down')}</button><div class="iconswitch" data-board-segments="true"><label>${icon('layout-grid')}</label><label>${icon('table')}</label></div><button class="fbtn" disabled>${icon('refresh-cw')}<span data-collapse-label>检查全部</span></button></div>`;
...
export function boardPageSkeleton(path: string, options: {followLayout?: string} = {}): string {
  ...
  if (path === '/stats') {
    ...
    body = `<div class="follow followmanage"><div class="fmanageoverview">${['关注创作者', '启用来源', '检查失败', '未看更新'].map(label => `<div><span>${label}</span><b>${line()}</b></div>`).join('')}</div>${segments(['关注列表', '添加关注', '来源和凭证'], 'follow-workspace-switch')}<div class="fmain"><section class="fsec" data-follow-workspace-panel="list">${followToolbar()}<div class="board-follow-selection"><label>${checkboxHtml('disabled')}全选本页</label>${table?'':followControl('全部收起')}</div><div class="frows fsources" data-layout="${table?'table':'default'}">${content}<div class="followpagefooter"><div class="followpageinfo">${line('120px')}${line('100px')}</div></div></div></section></div></div>`;
