...
import { nativeImageFit } from '../src/native-image';
...
describe('头像和标识的小图补底', () => {
  ...
  it('70 px 紧凑头像适用，32 px 图标不补底', () => {
    expect(nativeImageFit(32, 32, 70, 70).small).toBe(true);
  ...
  it('高像素密度屏幕上的 64 px 方标在紧凑模式按 32 CSS px 显示', () => {
    expect(nativeImageFit(64, 64, 70, 70, 2)).toEqual({ small: true, width: 32, height: 32 });
