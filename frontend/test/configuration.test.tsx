...
it('配置以通用媒体网络和维护分组，缺少通用选项时省略标题', () => {
  const {el}=mount({startup:{available:true,enabled:false,silent:true,message:'',desktop:false,desktop_message:''},peach_proxy:{mode:'direct',proxy_saved:false,needs_selection:false}});
...
describe('媒体文件夹列表', () => {
  it('Windows 只显示本机路径，CloudDrive 帮助跟随来源', async () => {
    ...
    const select = el.querySelector<HTMLElement>('.gselect:has([aria-label="媒体来源 1"])')!;
