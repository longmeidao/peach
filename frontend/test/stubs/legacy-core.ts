...
// @ts-expect-error 测试直接复用正式错误映射与有界并发：两者都是纯函数，不碰 DOM。
export {requestErrorMessage, icon, mapLimit} from '../../../web/js/core.js';
...
export const siteMarkUrl = (params: Record<string, string>): string =>
  `/site-mark?${new URLSearchParams(params)}`;
