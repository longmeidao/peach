...
BUDGET_CHANGES = {
    "AGENTS.md": [
        ...
        (88, "2026-09-08", "工作规则的事实条目并进 ADR 指针，常犯错误里的 shell 与路径下沉技能"),
    ],
    "docs/HANDOFF.md": [
        (260, "2026-08-17", "初始值"),
...
BYTE_BUDGET_CHANGES = {
    "AGENTS.md": [
        ...
        # 清退腾出来的余量留着不动，等于把位置空出来让散文回填——这个文件就是这么长到
        # 12 KB 的，ADR-0015 自己记着那个反例（涨到 82 行后预算在同一批里从 70 改成 90）。
        # 留 1 KB 是有用处的：真出现一条判据 1 的规则要能当场写进来，不必先走一次上调。
        # 一行 130～150 字节，1 KB 约等于 7 行。这个数不是测出来的最优值，它只是闸门。
        (11 * 1024, "2026-09-08", "清退后 10.0 KB，留 1 KB 给判据 1 的新规则，其余余量收回"),
    ],
    ...
    "docs/STATUS.md": [
        ...
        # 清退结果而不清退产出机制，就只是把下一次撞线推后两天：8 KiB 定下时文件 6,301 字节，
        # 两天后 8,146，涨的 1,845 字节里 1,701 来自「已核验能力」——那一节每上线一个特性加一行，
        # 永不减，因为「已核验」这个断言没有任何事件能让它失效。同期真正的运行态只涨了 241 字节：
        # 它的条目是原地替换的，边界由机器数、端口数和链路数封死。把只增不减的清单搬进
        # 按需读取的 docs/REUSE.md 之后，入口文件只剩运行态和一行批处理指针。
        (4 * 1024, "2026-09-05", "已定型的行为判据搬进 docs/REUSE.md，入口文件只留运行态，2.6 KB 上下"),
    ],
}
