...
用法:
  python scripts/traffic_watch.py                       阈值 50 GB，每 30 秒查一次
  python scripts/traffic_watch.py --limit 30 --interval 20
  python scripts/traffic_watch.py --once                查一次当前用量就退出
"""
...
ONCE = "--once" in A
# 触发时要杀掉的任务（读网盘的那些）
KILL_PATTERNS = ["scripts\\probe.py", "scripts\\sheets.py", "rm-probe.py", "rm-sheets.py"]
