...
being let in by accident. The token gate is off whenever `peach serve` runs without `--token`, so
binding to a LAN address requires a token. The service binds to loopback (`127.0.0.1`) by default
